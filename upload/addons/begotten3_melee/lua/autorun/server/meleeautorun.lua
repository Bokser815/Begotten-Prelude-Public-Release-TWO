...
local function Guarding(ent, dmginfo)
	...
	if (bIsPlayer and ent:Alive()) or (ent:IsNextBot() or ent:IsNPC()) and ent.GetActiveWeapon then
			...
			if canblock then
				...
				if (IsValid(attacker) and (math.abs(math.AngleDifference(ent:EyeAngles().y, (attacker:GetPos() - ent:GetPos()):Angle().y)) <= blockthreshold)) then
					...
					if bIsPlayer and !ent:GetNetVar("Deflect") and ent:Alive() and attacker:IsValid() then
						...
						if attacker:IsPlayer() then 
							local intoxication = attacker:GetNeed("intoxication")
							if intoxication and intoxication > 10 then
									if IsValid(dmginfo:GetInflictor()) and dmginfo:GetInflictor().isJavelin then
										-- nothing
									else
										poiseDamageModifier = poiseDamageModifier * Lerp(intoxication / 100, 1, 1.25);
									end
								end
						end
						...
						if attacker:IsPlayer() and enemyattacktable["poisedamage"] then
							...
							if ent.HasBelief then
								...
								if ent:HasBelief("sows_belief1") then
									local activeWeapon = ent:GetActiveWeapon();

									if string.find(activeWeapon.Category, "Two Handed") or string.find(activeWeapon.Category, "Great Weapon") or string.find(activeWeapon.Category, "Scythe") then
										newEnemyPoise = newEnemyPoise * 0.80;
									end
								end
		...
		elseif !ent.iFrames then
			...
			if enemywep:IsValid() and enemywep:GetClass() == "begotten_polearm_lungemine" then
				local weaponItemTable = item.GetByWeapon(enemywep);
				if weaponItemTable then

					local pos = dmginfo:GetDamagePosition()
					util.ScreenShake( pos, 700, 500, 1, 400 )
					local Forward = attacker:GetForward()

					local explosion = ents.Create("env_explosion");
					local explosionPos = pos
					...
					ent:SetVelocity(attacker:GetAimVector()*500)
					
					util.Decal("Scorch", pos + VectorRand(), pos - VectorRand())

					attacker:EmitSound("fiend/wtinessme.wav", 100, 100, 1, CHAN_AUTO)

					local player = attacker

					Clockwork.chatBox:AddInTargetRadius(player, "me", "shatters their thunder spear.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
				
					--self:Explosion()
					--weaponItemTable:TakeCondition(666);
					attacker:TakeItem(weaponItemTable)
				end
			end
			...
			if attacker:IsPlayer() then
				...
				if attacker:GetSubfaction() == "Varazdat" or attacker:GetSubfaction() == "Immortal" then
					if IsValid(enemywep) and enemywep.IsABegottenMelee then
				...
				elseif cwBeliefs and attacker.HasBelief and attacker:HasBelief("vultures_belief2") then
					if IsValid(enemywep) and enemywep.IsABegottenMelee then
						-- Scope pyramids ahoy!!!
						if attacker:HasBelief("vultures_belief3") then
							attacker:SetHealth(math.Clamp(math.ceil(attacker:Health() + (dmginfo:GetDamage() / 3)), 0, attacker:GetMaxHealth()));

							if attacker.swarmPartner then
								target = attacker.swarmPartner
								target:SetHealth(math.Clamp(math.ceil(target:Health() + (dmginfo:GetDamage() / 3)), 0, target:GetMaxHealth()));

								target:ScreenFade(SCREENFADE.OUT, Color(100, 20, 20, 80), 0.2, 0.1);
						
								timer.Simple(0.2, function()
									if IsValid(attacker) then
										target:ScreenFade(SCREENFADE.IN, Color(100, 20, 20, 80), 0.2, 0);
									end
								end);
							end
						else
							attacker:SetHealth(math.Clamp(math.ceil(attacker:Health() + (dmginfo:GetDamage() / 4)), 0, attacker:GetMaxHealth()));
				...
				-- stupid piggy regain stability
				elseif cwBeliefs and attacker.stupidFuckingPigWantsToStayUp then
					if IsValid(enemywep) and enemywep.IsABegottenMelee then
						attacker:SetCharacterData("stability", math.Clamp(ent:GetCharacterData("stability", max_stability) + (dmginfo:GetDamage() / 3), 0, max_stability));
						attacker:SetNWInt("stability", ent:GetCharacterData("stability", max_stability));
					end
				else
