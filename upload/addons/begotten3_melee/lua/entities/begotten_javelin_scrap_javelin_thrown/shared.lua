...
if SERVER then
	...
	function ENT:Initialize()
		...
		if self.Owner and self.Owner.transfertableready then
			self.AttackTable = self.Owner.transfertableready["AttTable"]
			self.BallistaType = self.Owner.transfertableready["BallType"]
			self.BoltConditionLoss = self.Owner.transfertableready["ConditionLoss"]
			self.CustomImpactEffect = self.Owner.transfertableready["CustomImpEff"]
			self.Owner.transfertableready = nil
		end
		...
		elseif self.BallistaType then
			if self.BoltConditionLoss then self.ConditionLoss = self.BoltConditionLoss end
			self.itemTable = item.CreateInstance(self.BallistaType);
		else
			self.itemTable = item.CreateInstance("begotten_javelin_scrap");
		end
			...
			if !self.collided then
				...
				if Ent:IsWorld() and self.InFlight then
					if data.Speed > 500 then
						...
						if self.BallistaType then
							self:EmitSound(Sound("prelude/ballista/ballista_impact_world_0"..math.random(1, 3)..".ogg"))
						else
					...
					if Clockwork and IsValid(self.Owner) and !self.Owner.opponent then
						if self.itemTable then
				...
				elseif Ent.Health and IsValid(self.Owner) then
					...
					if Ent:IsNPC() or Ent:IsNextBot() or (Ent:IsPlayer() and !Ent:GetNWBool("Parry") and !Ent:GetNWBool("Deflect")) and !Ent.iFrames then
						...
						if self.CustomImpactEffect then
							self.CustomImpactEffect(self:GetPos(),Ent)
						end
