...
SWEP.AttackTable = "ScrapJavelinAttackTable"

SWEP.Primary.Round = ("begotten_javelin_scrap_javelin_thrown");
...
function SWEP:HandlePrimaryAttack()
	...
	timer.Create("javelin_timer_"..self.Owner:EntIndex(), 0.5, 1, function()
		if IsValid(self) and IsValid(self.Owner) and self.Owner:GetActiveWeapon() and self.Owner:GetActiveWeapon().PrintName == "Scrap Javelin" then
			self:FireJavelin();
		end
	end);
...
function SWEP:FireJavelin()
	...
	if SERVER then
		...
		javelin:SetModel("models/lostcoast/fisherman/harpoon offset.mdl");
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_scrap");
			end
		else
...
SWEP.VElements = {
	["v_javelin"] = { type = "Model", model = "models/lostcoast/fisherman/harpoon offset.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(0.675, -0.519, -1.105), angle = Angle(105, 0, 82.986), size = Vector(0.75, 0.75, 0.75), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_javelin"] = { type = "Model", model = "models/lostcoast/fisherman/harpoon offset.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.9, 0.5, -7.5), angle = Angle(97.013, 40.909, 54.935), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElementsAlternate = {
	["w_javelin"] = { type = "Model", model = "models/lostcoast/fisherman/harpoon offset.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3, 1, -6.9), angle = Angle(-97, -110, 0), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}