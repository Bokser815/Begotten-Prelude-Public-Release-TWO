...
SWEP.HoldType = "wos-begotten_1h_throwable"
...
SWEP.Primary.Round = ("begotten_throwingdagger_assassin_thrown");
...
function SWEP:FireJavelin()
	...
	if SERVER then
		...
		javelin:SetModel("models/demonssouls/weapons/secret throwing dagger.mdl");
		...
		if itemTable then
			if self.Owner.opponent then
				...
				if self.Owner.StripWeapon then
					self.Owner:StripWeapon("begotten_prelude_throwingdagger_assassinn");
				end
			else
...
SWEP.VElements = {
	["v_javelin"] = { type = "Model", model = "models/demonssouls/weapons/secret throwing dagger.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(4.675, -0.519, -16.105), angle = Angle(105, 0, 82.986), size = Vector(0.75, 0.75, 0.75), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_javelin"] = { type = "Model", model = "models/demonssouls/weapons/secret throwing dagger.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.9, -0.5, -20), angle = Angle(97.013, 40.909, 54.935), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}