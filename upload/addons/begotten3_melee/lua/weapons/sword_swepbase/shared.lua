...
function SWEP:Hitscan()
	...
	if ( tr.Hit ) and !tr.Entity:IsPlayer() and !tr.Entity:IsNPC() then
		...
		if string.find(tr.Entity:GetClass(), "prop_ragdoll") then
			...
			local weptype = self.Owner:GetActiveWeapon()
			if weptype:IsValid() and weptype:GetClass() == "begotten_polearm_lungemine" then
				local weaponItemTable = item.GetByWeapon(weptype)
				if weaponItemTable then
					...
					if ( tr.Hit ) and (tr.Entity.LVS and tr.Entity.LVS== true) or (IsValid(tr.Entity) and !tr.Entity:IsPlayer()) then
						...
						explosion:SetKeyValue("iRadiusOverride", "1") 
						explosion:Spawn();
						explosion:Activate();
						explosion:Fire("Explode", "", 0);
						self.Owner:SetVelocity(self.Owner:GetAimVector() * -650)
					end
					...
					self.Owner:EmitSound("fiend/wtinessme.wav", 100, 100, 1, CHAN_AUTO)
				
					--self:Explosion()
					--weaponItemTable:TakeCondition(666);
					self.Owner:TakeItem(weaponItemTable)
...
function SWEP:PrimaryAttack()
	...
	if SERVER and bAttack and self:IsValid() and (!owner.IsRagdolled or !owner:IsRagdolled()) and owner:Alive() then
		...
		self:CreateTimer(strikeTime - math.Rand(0.2, 0.35), "NPCParryTimer"..owner:EntIndex(), function()
			...
			--print(tr.Entity);
			if(!IsValid(tr.Entity) or tr.Entity:IsPlayer() or tr.Entity:GetClass() != "npc_bgt_player" or tr.Entity.isBlocking) then return; end
			if(!tr.Entity.parryNext and math.random(1, Lerp(tr.Entity:GetNWInt("Aggressiveness")/50, 10, 3)) != 1) then return; end
			if(tr.Entity.nextCanParry and tr.Entity.nextCanParry > CurTime()) then return; end

			if(tr.Entity.isAttacking and math.random(1, 5) == 1) then
				tr.Entity.rollIn = 0;
			else
				tr.Entity.doParry = true;
...
--if (SERVER) then
	function SWEP:HandleHit(hit, src, swingType, hitIndex, offhandWeapon, offhandAttackTable)
		if !IsValid(self.Owner) then return end -- thunda
