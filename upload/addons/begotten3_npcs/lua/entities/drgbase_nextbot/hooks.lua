...
if SERVER then
	...
	function ENT:OnInjured(dmg)
		if dmg:GetInflictor().LVS==true then
			dmg:ScaleDamage(0.35);
			if !self.nextVehicleDamage then self.nextVehicleDamage = 0 end
			if self.nextVehicleDamage and (CurTime() > self.nextVehicleDamage) then
				dmg:ScaleDamage(0);
				self.nextVehicleDamage = CurTime() + 1;
			else
			  local damageamount = dmg:GetDamage()
			  local vehicle = dmg:GetInflictor()
			  local dmgInfo = DamageInfo()
			  
			  dmgInfo:SetDamage(damageamount/3)
