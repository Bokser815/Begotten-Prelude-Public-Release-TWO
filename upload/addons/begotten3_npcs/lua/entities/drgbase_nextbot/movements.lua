	...
	function ENT:FindLedge(propOnly)
		...
		if IsEntityClimbable(self, hull.Entity) then
			...
			for j = 1, sampleCount do
				local trDown = self:TraceHull(self:GetUp() * -999, { start = tr.HitPos + Vector(0, 0, j) })
				sumZ = sumZ + trDown.HitPos.z
			end
			local avgDownZ = sumZ / sampleCount

			if avgDownZ - self:GetPos().z > self.ClimbLedgesMaxHeight then 
				return 
			end
			...
			local ledge = self:TraceLine(trRad[1].Normal * self:Length(), {
				collisiongroup = COLLISION_GROUP_DEBRIS,
				start = Vector(pos.x, pos.y, avgDownZ - 1)
			}).HitPos
			local ledgeHeight = ledge.z - self:GetPos().z
			if math.Clamp(ledgeHeight, self.ClimbLedgesMinHeight, self.ClimbLedgesMaxHeight) == ledgeHeight then
				return ledge
			end
		end
	end
	...
	function ENT:ClimbLedge(ledge, callback)
		...
		if res ~= true then
			...
			while true do
				...
				local pos = lastPos + lastPos:DrG_Direction(ledge):GetNormalized() * self:GetSpeed() * self:GetScale() * dt
				if pos.z > ledge.z then 
					pos.z = ledge.z 
				end
	
				-- Incremental precision trace loop: use the computed pos as the base
				local precision = 5
				local i = 1
				local trCheck = { Hit = true, HitNonWorld = true }
				local adjustedPos = pos
				while trCheck.Hit do
					if i * precision > math.abs(ledge.z - pos.z) then
						break
					end
					trCheck = self:TraceHull(self:GetForward() * self:Length(), {
						start = pos + Vector(0, 0, i * precision)
					})
					if not trCheck.Hit then
						adjustedPos = pos + Vector(0, 0, i * precision)
						break
					end
					i = i + 1
				end
				...
				if not IsEntityClimbable(self, hull.Entity) then
					...
					self:SetPos(adjustedPos + offset)
					local remaining = math.abs(ledge.z - self:GetPos().z) / self:GetScale()
					if remaining == 0 then 
						break 
					end
					if self:OnClimbing(ledge, remaining, false) then 
						break 
					end
					if isfunction(callback) and callback(self, ledge, remaining, false) then 
						break 
					end
				end
			end
			...
			self:CustomClimbing(ledge, height, false)
		end
		self:SetNW2Bool("DrGBaseClimbing", false)
		self:SetVelocity(Vector(0, 0, 0))
	end
