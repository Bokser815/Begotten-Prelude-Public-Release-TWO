...
if SERVER then
	...
	--edgy fix: drgbase_nextbot/shared.lua

    local MAX_CONSECUTIVE_LEDGE = 7 -- number of tries before forcing a new patrol

    function ENT:Patrol()
        if not EnablePatrol:GetBool() then 
            return 
        end
        ...
        if not isbool(res) then
            ...
            if follow == "ledge" then
                self._ConsecutiveLedgeCount = (self._ConsecutiveLedgeCount or 0) + 1
                if self._ConsecutiveLedgeCount >= MAX_CONSECUTIVE_LEDGE then
                ...
                self._ConsecutiveLedgeCount = 0
                if follow == "unreachable" then 
                    res = false
                elseif follow == "reached" then 
                    res = true 
                end
            end
        end
