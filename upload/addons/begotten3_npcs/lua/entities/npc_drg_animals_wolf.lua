...
if SERVER then
  ...
  function ENT:Think()
    ...
    if not self._lastUnstuckCheck or curTime - self._lastUnstuckCheck >= 2 then
        self:Unstick()
        self._lastUnstuckCheck = curTime
        --print("[WOLVES] Attempting unstick")
    end
...
function ENT:OnIdle()
    ...
    if self.pack and #self.pack > 0 then
        center = Vector(0, 0, 0)
        local count = 0
        for k, v in ipairs(self.pack) do
            if IsValid(v) then
    ...
    local newPatrolPos = center + self:RandomPos(1000)
    self:AddPatrolPos(newPatrolPos)
    self:SetPlaybackRate(1)
    return true
...
function ENT:Unstick()
    ...
    local nearby = ents.FindInSphere(pos, 20)
    local sep = Vector(0, 0, 0)
    local count = 0

    for _, ent in ipairs(nearby) do
        if ent ~= self and ent:GetClass() == self:GetClass() then
            local diff = pos - ent:GetPos()
            local len = diff:Length()
            if len > 0 then
                sep = sep + diff:GetNormalized() * (20 - math.min(len, 20))
                count = count + 1
            end
        end
    end
    ...
    if count > 0 then
        sep = sep / count
        local pushAmount = 20
        local newPos = pos + sep * pushAmount
        self:SetPos(newPos)
        if self.loco and self.loco:IsStuck() then
            self:ClearStuck()
        end
    end
end
