...
function ENT:LVSHudPaintSeatSwitcher( X, Y, w, h, ScrX, ScrY, ply )
	...
	for _, Pod in pairs( pSeats ) do
		...
		if I == MySeat then
			...
			--draw.RoundedBox(5, X + xPos - xHider, yPos + I * 30, 35 + HiderOffset, 25, Color(LVS.ThemeColor.r, LVS.ThemeColor.g, 0, 11) )
		else
			--draw.RoundedBox(5, X + xPos - xHider, yPos + I * 30, 35 + HiderOffset, 25, Color(0,0,0,100 + 50 * ply.smHider) )
			--draw.RoundedBox(5, X + xPos - xHider, yPos + I * 30, 35 + HiderOffset, 25, Color(0,0,0,0) )
		end

		if Hide then
			if Passengers[I] then
				--draw.DrawText( Passengers[I], "LVS_FONT_SWITCHER", X + 40 + xPos - xHider, yPos + I * 30 + 2.5, Color( 220, 20, 60,  Alpha1 ), TEXT_ALIGN_LEFT )
			else
				--draw.DrawText( "-", "LVS_FONT_SWITCHER", X + 40 + xPos - xHider, yPos + I * 30 + 2.5, Color( 220, 20, 60,  Alpha1 ), TEXT_ALIGN_LEFT )
			end
			
			--draw.DrawText( "["..I.."]", "LVS_FONT_SWITCHER", X + 17 + xPos - xHider, yPos + I * 30 + 2.5, Color( 220, 20, 60, Alpha1 ), TEXT_ALIGN_CENTER )
		else
			if Passengers[I] then
				--draw.DrawText( "[^"..I.."]", "LVS_FONT_SWITCHER", X + 17 + xPos - xHider, yPos + I * 30 + 2.5, Color( 220, 20, 60, Alpha1 ), TEXT_ALIGN_CENTER )
			else
