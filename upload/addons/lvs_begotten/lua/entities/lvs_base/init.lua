...
function ENT:Touch(ent)
	if engine.ActiveGamemode() == "cwbegotten" then
		cwVehicleMelee:CustomCallTouched(ent,self)
	end
end
...
function ENT:IsUseAllowed( ply )
	...
	if (LVS.TeamPassenger:GetBool() and ((self:GetAITEAM() ~= ply:lvsGetAITeam()) and ply:lvsGetAITeam() ~= 0 and self:GetAITEAM() ~= 0)) then 
...
function ENT:SucessBegottenEnter( ply )
...
function ENT:Use( ply )
	...
	cwVehicleMelee:CustomEnter(ply, self)
	--[[
	if not istable( self._DoorHandlers ) then
		self:SetPassenger( ply )
...
function ENT:AddUpgrade(upgradeID, applier)
	self.appliedUpgrades = self.appliedUpgrades or {}
    if not table.HasValue(self.appliedUpgrades, upgradeID) and #self.appliedUpgrades < self.upgradeSlots then
        table.insert(self.appliedUpgrades, upgradeID)
        local upgrade = cwVehicleUpgrades:GetUpgrade(upgradeID)
        if upgrade and upgrade.apply then
            upgrade.apply(self, applier)
        end
		self:SetNWString("AppliedUpgrades", util.TableToJSON(self.appliedUpgrades)) --esoteric gpt knowledge says that this can be used to store tables as persistent networked variables and I listen to my kthonic deities
        print("[Vehicle] Added upgrade: " .. upgradeID)
    else
		if table.HasValue(self.appliedUpgrades, upgradeID) then
            print("[Vehicle] Upgrade already applied: " .. upgradeID)
			Schema:EasyText(applier, "red", "This vehicle already has this upgrade!")
        else
            print("[Vehicle] Maximum upgrade slots reached.")
			Schema:EasyText(applier, "red", "This vehicle has reached its maximum number of upgrade slots!")
        end
    end
end

-- Remove an upgrade from the vehicle.
function ENT:RemoveUpgrade(upgradeID)
    if self.appliedUpgrades then
        for i, id in ipairs(self.appliedUpgrades) do
            if id == upgradeID then
                table.remove(self.appliedUpgrades, i)
                local upgrade = cwVehicleUpgrades:GetUpgrade(upgradeID)
                if upgrade and upgrade.remove then
                    upgrade.remove(self)
                end
                print("[Vehicle] Removed upgrade: " .. upgradeID)
                break
            end
        end
		self:SetNWString("AppliedUpgrades", util.TableToJSON(self.appliedUpgrades))
    end
end
