...
function ENT:CreateBaseDT()
	...
	self:AddDT( "Float", "Fuel", { KeyName = "fuel", Edit = { type = "Float", order = 4,min = 0, max = self.MaxFuel or 100, category = "Misc"} } )
	...
	if SERVER then
		...
		self:NetworkVarNotify( "Fuel", self.PDSFuelValueChanged )
		if !self.MaxFuel then self.MaxFuel = 100 end
...
-- Retrieve the table of applied upgrades.
function ENT:GetUpgrades()
    if SERVER then
        return self.appliedUpgrades or {}
    else
        local data = self:GetNWString("AppliedUpgrades", "[]")
