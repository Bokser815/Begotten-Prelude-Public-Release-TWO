...
function ENT:PaintZoom( X, Y, ply )
	...
	surface.SetDrawColor( Color(179, 46, 49,255 * zoom) )
...
function ENT:LVSHudPaintInfoText( X, Y, W, H, ScrX, ScrY, ply )
	...
	--draw.DrawText( "km/h ", "BarTextBegotten", X + 72, Y + 35, Color(179, 46, 49, 255), TEXT_ALIGN_RIGHT )
	--draw.DrawText( kmh, "BarTextBegotten", X + 72, Y + 35, Color(179, 46, 49, 255), TEXT_ALIGN_LEFT )

	if ply ~= self:GetDriver() then return end

	local Throttle = self:GetThrottle()
	local Col = Throttle <= 1 and Color(179, 46, 49, 255) or Color(0,0,0,255)
	local hX = X + W - H * 0.5
	...
	surface.SetDrawColor( 179, 46, 49, 255 )
	...
	if not self:GetEngineActive() then
		...
		if WaveScale > 0 then
			...
			surface.SetDrawColor( 179, 46, 49, 255 * WaveScale ^ 2 )
...
function ENT:LVSHudPaintCarMenu( X, Y, w, h, ScrX, ScrY, ply )
	...
	if MenuOpen then
		...
		if IsValid( LightsHandler ) then
			if LightsHandler:GetActive() then
				if LightsHandler:GetHighActive() then
					...
					DrawTexturedRect( cX, cY, size, Color(179, 46, 49, 255) )
				else
			...
			if LightsHandler:GetFogActive() then
				...
				DrawTexturedRect( cX, cY - (size + dist), size, Color(179, 46, 49, 255) )
			end
		end
		...
		if TurnMode == 1 or TurnMode == 3 then
			...
			DrawTexturedRect( cX - (size + dist), cY, size, Color(179, 46, 49,Alpha) )
		end
...
function ENT:DrawDeveloperInfo()
	...
	draw.SimpleTextOutlined( "velocity u/s", "DermaDefault", X + SizeX + 15, Y + SizeY, Color(179, 46, 49, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, color_black )
	draw.SimpleTextOutlined( self:GetReverse() and "torque@wheel [reverse]" or "torque@wheel", "DermaDefault", X, Y - 15, Color(179, 46, 49, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black )
	...
	for a = 0, target, 250 do
		...
		draw.SimpleTextOutlined( a, "DermaDefault", X1, Y1 + 10, Color(179, 46, 49, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black )
	end
	...
	for a = 0, SizeY, 20 do
		...
		draw.SimpleTextOutlined( a * 5, "DermaDefault", X1 - 10, Y1, Color(179, 46, 49, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, color_black )
	end
end
