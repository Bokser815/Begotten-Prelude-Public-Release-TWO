...
local fueldrainrates = {
	[3] = 0.01, -- high fuel drain
	[2] = 0.005, -- medium fuel drain
	[1] = 0.001, -- low fuel drain
}
...
function ENT:CalcThrottle( ply )
	...
	if Vel:Length() > 100 and self:GetEngineActive()==true then
		local fueldrain = self.assignedfueldrain or 1
		if !self.currentfueldrainrate then self.currentfueldrainrate = fueldrainrates[fueldrain] end
		local fuel = math.Approach( self:GetFuel(), 0, self.currentfueldrainrate )
		self:SetFuel( fuel )

		if !self.lasttimechangedfueldrain then self.lasttimechangedfueldrain = CurTime() end
		if CurTime()>=self.lasttimechangedfueldrain then
			self.lasttimechangedfueldrain = CurTime() + 1
			self.currentfueldrainrate = fueldrainrates[fueldrain]
		end
	end
