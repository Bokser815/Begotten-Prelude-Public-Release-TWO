...
function ENT:OnSpawn( PObj )
	--[[ basics ]]
	local driver = self:AddDriverSeat( Vector(15,0,8), Angle(0,90,0) ) -- self:AddDriverSeat( Position,  Angle ) -- add a driver seat (max 1)
	driver.ExitPos = Vector(0,-55,15) -- change exit position
	driver.HidePlayer = false -- should the player in this pod be invisible?

	--[[ engine sound / effects ]]
	self:AddEngine( Vector(45,0,48),Angle(0,180,0) ) -- add a engine. This is used for sounds and effects and is required to get accurate RPM for the gauges.
	self:AddFuelTank( Vector(0,0,0), Angle(0,180,0), 500, LVS.FUELTYPE_DIESEL, Vector(0,0,0), Vector(0,0,0) )
	--[[ fuel system ]]
	...
	local WheelFrontLeft = self:AddWheel( { mdl_ang = Angle(0,0,0),pos = Vector(-65,35,10), mdl = WheelModel } )
	local WheelFrontRight = self:AddWheel( { mdl_ang = Angle(0,180,0), pos = Vector(-65,-35,10), mdl = WheelModel } )

	local WheelRearLeft = self:AddWheel( {mdl_ang = Angle(0,0,0), pos = Vector(40,35,10), mdl = WheelModel} )
	local WheelRearRight = self:AddWheel( {mdl_ang = Angle(0,180,0), pos = Vector(40,-35,10), mdl = WheelModel} )
