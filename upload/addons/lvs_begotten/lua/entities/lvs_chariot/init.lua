...
function ENT:OnSpawn( PObj )
	--[[ basics ]]
	local driver = self:AddDriverSeat( Vector(50,0,30), Angle(0,90,-15) ) -- self:AddDriverSeat( Position,  Angle ) -- add a driver seat (max 1)
	driver.ExitPos = Vector(85,0,15) -- change exit position
	driver.HidePlayer = false -- should the player in this pod be invisible?

	local Pod = self:AddPassengerSeat( Vector(50,0,10), Angle(0,90,0) ) -- add a passenger seat (no limit)
	Pod.ExitPos = Vector(85,0,15) -- change exit position
	Pod.HidePlayer = false -- should the player in this pod be invisible?
	
	--[[ engine sound / effects ]]
	self:AddEngine( Vector(-55,0,35),Angle(0,180,0) ) -- add a engine. This is used for sounds and effects and is required to get accurate RPM for the gauges.
	...
	local WheelFrontLeft = self:AddWheel( { mdl_ang = Angle(0,0,0),pos = Vector(-56,30,5), mdl = "models/diggercars/kubel/kubelwagen_wheel.mdl" } )
	local WheelFrontRight = self:AddWheel( { mdl_ang = Angle(0,180,0), pos = Vector(-56,-30,5), mdl = "models/diggercars/kubel/kubelwagen_wheel.mdl" } )

	local WheelRearLeft = self:AddWheel( {mdl_ang = Angle(0,0,0), pos = Vector(40,30,10), mdl = WheelModel} )
	local WheelRearRight = self:AddWheel( {mdl_ang = Angle(0,180,0), pos = Vector(40,-30,10), mdl = WheelModel} )
