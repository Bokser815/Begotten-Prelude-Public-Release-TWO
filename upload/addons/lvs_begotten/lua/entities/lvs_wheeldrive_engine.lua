-- File: lua/entities/lvs_wheeldrive_engine.lua
...
-- SERVER and CLIENT shared below
...
function ENT:GetUpgrades()
    local upgrades = {}

    if self.GetTurbo and self:GetTurbo() and IsValid(self:GetTurbo()) then
        table.insert(upgrades, "Turbo")
    end

    if self.GetCompressor and self:GetCompressor() and IsValid(self:GetCompressor()) then
        table.insert(upgrades, "Supercharger")
    end

    if self.AppliedUpgrades and type(self.AppliedUpgrades) == "table" then
        for _, v in ipairs(self.AppliedUpgrades) do
            if cwVehicleUpgrades then
                local upg = cwVehicleUpgrades:GetUpgrade(v)
                if upg then
                    table.insert(upgrades, upg.name)
                end
            end
        end
    end
...
function ENT:ApplyUpgrade(upgradeID)
    if not self.AppliedUpgrades then self.AppliedUpgrades = {} end
    if not table.HasValue(self.AppliedUpgrades, upgradeID) then
        table.insert(self.AppliedUpgrades, upgradeID)
        print("[Engine] Applied upgrade: " .. upgradeID)
    else
        print("[Engine] Upgrade already applied: " .. upgradeID)
    end
end
    ...
    if self.AppliedUpgrades then
        for i, id in ipairs(self.AppliedUpgrades) do
            if id == upgradeID then
                ...
                print("[Engine] Removed upgrade: " .. upgradeID)
                break
            end
        end
    end
