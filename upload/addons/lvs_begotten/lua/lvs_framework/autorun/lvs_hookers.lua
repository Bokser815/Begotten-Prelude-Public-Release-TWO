...
hook.Add( "VehicleMove", "!!!!lvs_vehiclemove", function( ply, vehicle, mv )
	...
	if (ply._lvsOldThirdPerson ~= KeyThirdPerson) and isadmin == true then
...
hook.Add("EntityTakeDamage", "!!!!!!_lvs_begotten_fix_vehicle_damage", function(entity, dmgInfo) 
	if(!entity:IsPlayer()) then return; end

	local veh = entity:lvsGetVehicle();
	if(!IsValid(veh)) then return; end

	local inflictor = dmgInfo:GetInflictor();
	if(!IsValid(inflictor) or !string.find(inflictor, "lvs_")) then return; end

	if(veh == inflictor) then return true; end
