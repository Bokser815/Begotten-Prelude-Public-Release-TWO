...
function ENT:IsEnemy(ent)
	...
	if ent.IsVJBaseSNPC then
		...
		--Check their class table
		--VJ has so many damn classes that the presence of the word "friendly" is what we'll go by

		for _,v in pairs(ent.VJ_NPC_Class) do
			if isstring(v) then
				if string.find(string.lower(v), "friendly") != nil then
					return false
				end
			end
		end
