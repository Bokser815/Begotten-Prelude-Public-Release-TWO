util.PrecacheSound("gatling/gatling_close_shot_4.mp3")
...
hook.Add("EntityTakeDamage","DropChopper_FixCSRagdolls", function(ent, dmginfo)
    if !GetConVar("ai_serverragdolls"):GetBool() && ent.inHelicopter then
        ...
        if dmginfo:GetDamage() >= ent:Health() then
            ...
            if IsValid(ent) and ent:GetClass()!="npc_bgt_drop_chopper_evil" then
                ParticleEffectAttach("doom_dissolve", PATTACH_POINT_FOLLOW, ent, 0);
                
                timer.Simple(1.6, function() 
                    if IsValid(ent) then
                        ParticleEffectAttach("doom_dissolve_flameburst", PATTACH_POINT_FOLLOW, ent, 0);
                        ent:EmitSound("begotten/npc/burn.wav");
                        ent:Remove()
...
function ENT:EvacCrew()
    ...
    timer.Simple(6, function()
        ...
        if despawn_time > 0 then

            timer.Simple(despawn_time, function()
                if !IsValid(self) then return end
                self:Remove()
            end)
        end
        ...
        local mdl_translate = {
            ["models/kali/characters/bo/choppercrew/choppercrew_01.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_02.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_03.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_04.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_05.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_06.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_07.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_08.mdl"] = "models/begotten/thralls/suitor.mdl",
            ["models/kali/characters/bo/choppercrew/choppercrew_09.mdl"] = "models/begotten/thralls/suitor.mdl",
        }
        ...
        for k,v in pairs(to_evac) do
            ...
            if IsValid(v) && istable(placement_data) then
                local soldier
                if self.IsEvil then
                    soldier = ents.Create("npc_bgt_suitor")
                else
                    soldier = ents.Create("npc_bgt_suitor")
                end
                ...
                --soldier:SetModel("models/begotten/thralls/suitor.mdl")
...
function ENT:PilotsAlive()
    -- if true then return !self.PAOVERRIDE end
...
hook.Add("EntityEmitSound", "SilentCombineHurt", function(data)
    if IsValid(data.Entity) and data.Entity:GetClass() == "npc_combine_s" and data.Entity.inHelicopter then
        return false
    end
end)
...
function ENT:DeployCrew()
    ...
    for k,v in pairs(self.crew) do
        ...
        if IsValid(v) && istable(placement_data) then
            ...
            --soldier.NPCTable = {}
            --soldier.NPCTable.Name = v.specialty
            ...
            for i=0,#v:GetMaterials() do
                soldier:SetSubMaterial(i, v:GetSubMaterial(i))
            end

            if v.specialty == "Medic" then
                soldier:SetKeyValue("spawnflags", 132616)
            end
            ]]
            v:Remove()
        end
...
function ENT:LoadGuy(seat)
    ...
    guy:SetModel("models/begotten/thralls/suitor.mdl")
    guy:SetParent(self)
    ...
    guy.specialty = class.spec
    guy.NPCTable = {}
    guy.NPCTable.Name = "Helicopter "..class.spec
	...
	timer.Create(tName, 0, 0, function()
		if !IsValid(guy) || !IsValid(self) then timer.Remove(tName) return end
        
		guy:SetSequence(guy.sequence)
        guy:ResetSequenceInfo()
        guy:SetCycle( 0 )
        guy:SetPlaybackRate(0)
...
function ENT:SpawnGunner(right)
    ...
    guy.FireWeapon = function(this)
        ...
        this:EmitSound("gatling/gatling_close_shot_4.mp3",140,90,1,CHAN_WEAPON)
	...
	timer.Create(tName, 0, 0, function()
        ...
        if !IsValid(guy.target) then
            ...
            if guy.target:IsPlayer() && GetConVar("ai_ignoreplayers"):GetBool() then 
                guy.target = false
                return
            end
            ...
            if los.Entity == guy.target && Distance<600 && Yaw > -60 && Yaw < 70 then
                guy:FireWeapon()
            end
...
function ENT:SpawnPilot(isCopilot)
    ...
    if self.IsEvil then
        ...
        self:SetSubMaterial(5,"models/props_pipes/pipeset_metal")
