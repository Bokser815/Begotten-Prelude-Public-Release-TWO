...
function ENT:SmackBlade(hitpos, mat, ent)
	...
	if self.destruction.engine then
		self:BreakRotor()
	elseif !self:PilotsAlive() || self.destruction.tail || self.destruction.tail_rotor || self.destruction.fuel then
...
function ENT:RuptureFuel()
	...
	local burnOutTime = math.random(15,30)

	timer.Simple(burnOutTime, function()
		if !IsValid(self) then return end
		self:KillEngine()
		self:SetColor(Color(79,79,79))
	end)

	timer.Simple(burnOutTime+math.random(5,10), function()
		if !IsValid(self) then return end
		self:Explode()
	end)
...
function ENT:BreakTailRotor()
	if self.destruction.tail_rotor then return end
	self.destruction.tail_rotor = true
	self:SendDamageMessage("tail_rotor")
	for i = 1,20 do
	...
	local fxPos = vecMod(self:GetPos(), self:GetAngles(), Vector(-360,6,120))
...
function ENT:RotorGibs()
	...
	local g0 = ents.Create("drop_chopper_rotor_gib")
		...
		g0:SetMaterial("models/props_pipes/pipeset_metal")
	...
	local g1 = ents.Create("drop_chopper_rotor_gib")
		...
		g1:SetMaterial("models/props_pipes/pipeset_metal")
	...
	local g2 = ents.Create("drop_chopper_rotor_gib")
		...
		g2:SetMaterial("models/props_pipes/pipeset_metal")
...
function ENT:BreakTail(fling)
	...
	self.TailGib = gib
	gib:SetMaterial("models/props_pipes/pipeset_metal")
	...
	if fling then
		...
		if IsValid(phys) then
			phys:ApplyForceCenter(gib:GetAngles():Right()*1000000)
		end
	else
...
function ENT:PhysicsCollide( data, physobj )
	...
	if data.HitEntity:GetClass() == "npc_bgt_drop_chopper_evil" then return end
