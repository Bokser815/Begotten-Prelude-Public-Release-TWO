...
function ENT:UpdateAltitude()
	...
	if groundAltitude < 170 && !self.destruction.engine && !self.destruction.rotor then
		...
		effectdata2:SetScale(math.Clamp( (140 - groundAltitude)/10, 0, 30 ))
...
function ENT:CollisionAvoidance()
	...
	if self.destruction.fuel || !self:PilotsAlive() then return false end
	...
	local function bumper(ep, isForward)
		...
		if los.Hit then
			if !self.LastGoForward then self.LastGoForward=0 end
			local sidevec = los.HitNormal:Angle():Right()
			if isForward and self.LastGoForward and self.LastGoForward > CurTime() - 1 then
...
function ENT:FaceTarget(isBase)
	...
	if self.destruction.tail || self.destruction.tail_rotor then return end
...
function ENT:NegateSpin()
	...
	if self.destruction.tail || self.destruction.tail_rotor || !self:PilotsAlive() then return end
