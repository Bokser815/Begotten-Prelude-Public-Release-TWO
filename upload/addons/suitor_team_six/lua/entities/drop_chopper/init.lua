...
function ENT:Initialize()
	...
	self:SetNWEntity("pilot1", self.pilot1)
	self:SetNWEntity("pilot2", self.pilot2)
...
function ENT:PhysicsUpdate(phys)
	...
	if !self.destruction.engine then
		...
		if self.destruction.tail || !self:PilotsAlive() || self.destruction.tail_rotor then
	...
	if !self.destruction.tail && !self.destruction.tail_rotor then
