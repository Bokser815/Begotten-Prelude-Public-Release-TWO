...
SWEP.WorldModel = "models/weapons/v_smg_mothgun.mdl"
...
function SWEP:DrawWorldModel()
	...
	if IsValid(self.Owner) then
		bName = self.Owner:LookupBone("ValveBiped.Bip01_R_Hand")
	end

	if IsValid(self.Owner) && bName then
		...
		render.Model({
			model = "models/weapons/v_smg_mothgun.mdl",
            pos = vecMod(bPos, bAng, Vector(7,1,-2)),
            angle = angMod(bAng, Angle(180,10,90))
		})
	else
		render.Model({
			model = "models/weapons/v_smg_mothgun.mdl",
			pos = self:GetPos(),
			angle = angMod(self:GetAngles(), Angle(0,0,180))
		})
	end
...
function SWEP:PrimaryAttack()
	...
	local bName = enemy:LookupBone("ValveBiped.Bip01_Head1")
	if enemy:GetClass() == "npc_fastzombie" then
		bName = enemy:LookupBone("ValveBiped.HC_BodyCube")
	end
	if bName then
		local headPos, headAng = enemy:GetBonePosition(bName)
		targetpos = headPos
	else
		targetpos = LocalToWorld(enemy:OBBCenter(), Angle(), enemy:GetPos(), Angle() )
	end
	...
	effectdata:SetOrigin(vecMod(atDat.Pos,atDat.Ang, Vector(30,3,0)))
	...
	effectdata2:SetAngles(angMod(shelldat.Ang, Angle(0,0,45)))
