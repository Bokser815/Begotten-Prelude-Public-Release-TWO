...
local COMMAND = Clockwork.command:New("CharTransferSubfaction");
	...
	COMMAND.text = "<string Name> <string Subfaction> [bool Override]";
	COMMAND.access = "o";
	COMMAND.arguments = 3;
	COMMAND.optionalArguments = 1;
		...
		if (target) then
			...
			if (factionSubfactions) then
				...
				elseif(arguments[3]) then
					...
					Clockwork.player:LoadCharacter(target, Clockwork.player:GetCharacterID(target));
					Clockwork.player:NotifyAll(player:Name().." has transferred "..name.." to the "..subfaction.." custom subfaction.");
