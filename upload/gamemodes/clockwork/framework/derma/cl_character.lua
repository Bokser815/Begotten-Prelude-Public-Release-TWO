...
function PANEL:Init()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		...
		local subfactionTable;

		if(factionTable.subfactions) then
			for _, v in pairs(factionTable.subfactions) do
				if(v.name == Clockwork.Client.SelectedSubfaction) then subfactionTable = v; end
		...
		if (Clockwork.faction.stored[self.info.faction].useFullName) then
			...
			self.surnameRandomButton.DoClick = function()
				if RANDOM_SURNAMES and randomNames then
...
function PANEL:Init()
	...
	elseif self.curFaction == "Darklanders" then
		self.subfactionForm:SetName("Spicehands");
	elseif self.curFaction == "Wanderer" then
		self.subfactionForm:SetName("Heritage");
	end
