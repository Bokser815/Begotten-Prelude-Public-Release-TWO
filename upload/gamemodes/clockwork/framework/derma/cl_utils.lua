...
function Derma_ColorRequest(strTitle, strText, fnEnter)
    -- Create a frame for the color picker
    local Window = vgui.Create("DFrame")
    Window:SetTitle(strTitle or "Pick a Color") -- Use the provided title
    Window:SetDraggable(false)
    Window:ShowCloseButton(false)
    Window:SetBackgroundBlur(true)
    Window:SetDrawOnTop(true)
    ...
    -- Add a label for instructions (optional)
    local Text = vgui.Create("DLabel", InnerPanel)
    Text:SetText(strText or "Select a color below:")
    Text:SizeToContents()
    Text:SetContentAlignment(5)
    Text:Dock(TOP)
    ...
    -- Create the DColorMixer for color selection
    local colorMixer = vgui.Create("DColorMixer", InnerPanel)
    colorMixer:Dock(TOP)
    colorMixer:SetPalette(false)
    colorMixer:SetAlphaBar(false)
    colorMixer:SetWangs(false)
    colorMixer:SetTall(175)
    ...
    -- OK button to confirm the color
    local Button = vgui.Create("DButton", ButtonPanel)
    Button:SetText("OK") -- Use the provided OK button text
    Button:SetTall(20)
    Button:Dock(LEFT)
    Button:DockMargin(5, 5, 5, 5)
    Button.DoClick = function()
        local color = colorMixer:GetColor() -- Get the selected color
        Window:Close()
        if fnEnter then
            fnEnter(color)
        end
    ...
    ButtonCancel:DockMargin(5, 5, 5, 5)
