...
function GM:TopLevelPlayerBindPress(player, bind, bPress)
	...
	elseif (string.find(bind, "speed") and player:KeyDown(IN_WALK)) then
...
function GM:AdjustMouseSensitivity()
	if(!IsValid(Clockwork.Client)) then return -1; end

	local sensitivity = {1};
	hook.Run("ClockworkAdjustMouseSensitivityTable", sensitivity);

	local wep = Clockwork.Client:GetActiveWeapon()
	if(wep and wep.ClockworkAdjustMouseSensitivityTable) then
		wep:ClockworkAdjustMouseSensitivityTable(sensitivity);
...
function GM:MenuItemsAdd(menuItems)
	...
	local covenantName = Clockwork.option:GetKey("name_covenantsmenu")
	...
	menuItems:Add(covenantName, "cwCovenantsMain", "View details about your covenant.")
...
function Clockwork.kernel:SetupDermaToolTip(parent, subTooltip)
	...
	function frame:CloseTooltip()
		...
		self:Close();
		self:Remove();
		if(subTooltip) then Clockwork.kernel:ClearSubToolTipPanel(parent); else Clockwork.kernel:ClearToolTipPanel(parent); end
	end;
	...
	function frame:Think()
		...
		if (!self.open and !input.IsKeyDown(KEY_LSHIFT)) then
			self:SetPos(x, y);
		end;
		...
		if (!Clockwork.Client:IsRagdolled(RAGDOLL_FALLENOVER)) then
			...
			for k, v in pairs (self.TargetEntities) do
				...
				if alpha > 0 then
					...
					if(player and player:GetNetVar("isThrall")) then return; end
						...
						if (Clockwork.plugin:Call("ShouldDrawPlayerTargetID", player)) then
							if (!Clockwork.player:IsNoClipping(player)) then
								...
								if Clockwork.ConVars.PHYSDESCINSPECT:GetInt() == 0 or input.IsKeyDown(KEY_X) then
									...
									y = Clockwork.plugin:Call("DrawTargetPlayerCovenant", player, alpha, x, y) or y -- Added by cwWandererCovenants
...
function GM:GetTargetPlayerText(player, targetPlayerText)
	...
	if(player:GetNetVar("isThrall")) then return; end
...
function GM:HUDDrawScoreBoard()
	...
	elseif (self.DataStreamedAlpha and self.DataStreamedAlpha > 0) then
		local textString = "Please wait while Begotten: Prelude initializes.";
