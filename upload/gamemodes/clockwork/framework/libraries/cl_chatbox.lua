...
Clockwork.chatBox.AllowedLanguages = { -- TODO: Add languages gmod supports naturally.
    ["en"] = true, -- English
    ["ru"] = true, -- Russian
	["uk"] = true, -- Russian
    -- make sure the prefix is valid please otherwise it will break the translator and likley return unpredictable errors.
}
local ALS = Clockwork.chatBox.AllowedLanguages

function Clockwork.chatBox:TranslateTextToLanguage(upcomingkeyvalue, spotintable ,text, languageToTranslateInto, player, table, callback)
    if string.Trim(text) == "" then
        return -- if the text is empty then there is no point in translating it.
    end
	local entextreturn = nil
    if not ALS[languageToTranslateInto] then -- prohibits the use of invalid language prefixes or ones that could possibly return errors.
        --print("Invalid Language Prefix: " .. languageToTranslateInto)
        return
    end

	languageToTranslateInto = GetConVar("gmod_language"):GetString()
	--print(languageToTranslateInto)

    local subURL = text
	subURL = string.gsub(subURL, "\n", "\r\n")
	subURL = string.gsub(subURL, "([^%w %-%_%.%~])", function(c) return string.format("%%%02X", string.byte(c)) end)
	subURL = string.gsub(subURL, " ", "+")
	--[[
	if languageToTranslateInto == "ru" then
		--subURL = string.Replace(text, " ", "%20")
		--subURL = string.Replace(subURL, ".", "")
		--subURL = string.Replace(subURL, "?", "")
		--subURL = string.Replace(subURL, "!", "")
		--subURL = string.gsub(subURL, "%p", "")
		subURL = string.gsub(subURL, "([^%w %-%_%.%~])", function(c) return string.format("%%%02X", string.byte(c)) end)
		subURL = string.gsub(subURL, "\n", "\r\n")
		subURL = string.gsub(subURL, " ", "+")
	end
	if languageToTranslateInto == "en" then
		--subURL = string.gsub(subURL, "%p", "")
		subURL = string.gsub(subURL, "\n", "\r\n")
        subURL = string.gsub(subURL, "([^%w %-%_%.%~])", function(c) return string.format("%%%02X", string.byte(c)) end)
		subURL = string.gsub(subURL, " ", "+")
    ...
    local url = string.format("https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=%s&dt=t&q=%s", languageToTranslateInto, subURL) -- faster than concatenation apparently.
	local translatethisshit = nil
    http.Fetch(url, 
    function(body, len, headers, code) -- success function
        local json = util.JSONToTable(body)
		--player.lastmessageinchat = json[1][1][1]
		translatethisshit = json[1][1][1]
		if !Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue] then Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue] = {} end
		Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue][spotintable] = translatethisshit
		--print(translatethisshit)
		--print(spotintable)
    end,

    function(error) -- error function
		if !Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue] then Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue] = {} end
		Clockwork.Client.TranslationIndependentTables[upcomingkeyvalue][spotintable] = "-Error Translating Text-"
		print("Error Translating Text: ")
        print(error)
    end)
end

function Clockwork.chatBox:GetClientLanguage()
    return ( GetConVar("gmod_language"):GetString() == "en-PT" and "en" ) or GetConVar("gmod_language"):GetString() -- en-PT is the language code for Pirate which we will consider english lol.
end
		...
		if (!Clockwork.kernel:IsChoosingCharacter() and !Clockwork.Client.LoadingText) then
			if (name != "") then
				...
				elseif faction == "Goreic Warrior" then
					fontOverride = "Gore";
				elseif faction == "Piston's Scrappers" then
					fontOverride = "Scrapper";
				elseif faction == "Darklanders" and !speaker:GetSharedVar("southlander") or subFaction == "Darklander" then
					fontOverride = "Darklander";
...
function Clockwork.chatBox:Paint()
	...
	local chatBoxTextDarklanderFont = Clockwork.option:GetFont("chat_box_text_darklander");
	local chatBoxTextMordredFont = Clockwork.option:GetFont("chat_box_text_mordred")
	local bIsOpen = Clockwork.chatBox:IsOpen();

	if Clockwork.Client.TranslationCleanupTimer and #Clockwork.Client.TranslationIndependentTables>0 and Clockwork.Client.TranslationCleanupTimer+30 < CurTime() then
		Clockwork.Client.TranslationIndependentTables = {}
	end
--[[
	if (bIsOpen) then
...
-]]
	...
	for k, v in pairs(messages) do
		...
		if v.font then
			...
			elseif v.font == "Darklander" then
				fontName = Clockwork.fonts:GetMultiplied(chatBoxTextDarklanderFont, v.multiplier or 1)
		...
		if !Clockwork.Client.NextTimeToCheckForTranslationServices then Clockwork.Client.NextTimeToCheckForTranslationServices = CurTime() end
		local shouldwetranslate = GetConVar( "translateEnabled" )
		local ourcountrycode = Clockwork.Client:GetNetVar("CountryCode")
		if v.playernationality then
			local langcode1 = chatlanguagegroups[v.playernationality]
			local langcode2 = chatlanguagegroups[ourcountrycode]
		end
		if (Clockwork.Client.NextTimeToCheckForTranslationServices<CurTime()) and shouldwetranslate and shouldwetranslate:GetInt() == 1 and v.playernationality and v.playernationality!=ourcountrycode and langcode1!=langcode2 then
			if v.text and !v.assignedtexttranslation then
				if !Clockwork.Client.SelectedLanguage then 
					Clockwork.Client.SelectedLanguage = Clockwork.chatBox:GetClientLanguage() 
				end
				if !v.numberoftranslated then v.numberoftranslated = 0 end
				if !Clockwork.Client.TranslationIndependentTables then Clockwork.Client.TranslationIndependentTables = {} end
				for i = 1,#v.text do
					v.numberoftranslated = v.numberoftranslated+1
					local uniquekey = #Clockwork.Client.TranslationIndependentTables+1
					v.keyforfetch = uniquekey
					Clockwork.chatBox:TranslateTextToLanguage(uniquekey,i,v.text[i]["text"],"ru",Clockwork.Client,v)
					
					--print(v.text[i]["text"])
					--PrintTable(v)
				end
				v.assignedtexttranslation = true

				Clockwork.Client.TranslationCleanupTimer = CurTime()
			end
			if !v.notranslationcheckcount or v.notranslationcheckcount < v.numberoftranslated then
				if !v.checkedvalues then v.checkedvalues = {} end
				for i = 1,#v.text do
					if !v.checkedvalues[i] then
						if !Clockwork.Client.TranslationIndependentTables then Clockwork.Client.TranslationIndependentTables = {} end
						if Clockwork.Client.TranslationIndependentTables[v.keyforfetch] then
							if Clockwork.Client.TranslationIndependentTables[v.keyforfetch] then
								if Clockwork.Client.TranslationIndependentTables[v.keyforfetch][i]!=nil and Clockwork.Client.TranslationIndependentTables[v.keyforfetch][i]!="" then
									--print("found",i) -- checking same v.text twice and raising our value which causes us to stop checking fix this
									v.checkedvalues[i] = true
									v.text[i]["text"] = Clockwork.Client.TranslationIndependentTables[v.keyforfetch][i]
									if v.notranslationcheckcount then v.notranslationcheckcount = v.notranslationcheckcount+1 end
									if !v.notranslationcheckcount then v.notranslationcheckcount = 1 end
									if v.notranslationcheckcount == v.numberoftranslated then
										--print(v.text[i]["text"])
									end
								end
							end
		...
		elseif (Clockwork.Client.NextTimeToCheckForTranslationServices<CurTime()) then
			Clockwork.Client.NextTimeToCheckForTranslationServices = CurTime()+1
		end
	...
	if (!filtered) then
		...
		if (text) then
			...
			for k, v in pairs(text) do
				...
				elseif v == "Gore" then
					message.font = "Gore";
				elseif v == "Darklander" then
					message.font = "Darklander"
				...
				elseif (type(v) == "Player") then
					...
					curColor = nil;
					curOnHover = nil;
					message.playernationality = v:GetNetVar("CountryCode")
				elseif (type(v) == "table") then
