...
if (SERVER) then
	...
	function Clockwork.dermaRequest:RequestColor(player, title, question, default, Callback)
		local rID = self:GenerateID()
		netstream.Start(player, "dermaRequest_colorQuery", {id = rID, title = title, question = question, default = default})
		hooks[rID] = {Callback = Callback, player = player}
	end
	...
	netstream.Hook("dermaRequest_colorQuery", function(data)
		Derma_ColorRequest(data.title, data.question, function(recv)
			Clockwork.dermaRequest:Send(data.id, recv)
		end)
	end)
