		...
		if (itemTable) then
			...
			elseif (itemAction == "breakdown") then
				if (itemTable.components) then
					local components = itemTable.components

					--TODO: come back and see if this permanently reassigns the components table
					-- if yes, fix this shit. use a variable to store components instead and rewrite the variable
					-- from vlad

					if player:GetFaction() == "Piston's Scrappers" and components.scrapperItems then
						components.items = components.scrapperItems;
					end


					if components.breakdownType == "meltdown" then
							...
							if !cwBeliefs or (cwBeliefs and player:HasBelief("smith")) then
								...
								for i = 1, #cwRecipes.smithyLocations do
									if player:GetPos():DistToSqr(cwRecipes.smithyLocations[i]) < (256 * 256) then
										...
										for j = 1, components.items do
											local componentItem = item.CreateInstance(components.items[j]);
