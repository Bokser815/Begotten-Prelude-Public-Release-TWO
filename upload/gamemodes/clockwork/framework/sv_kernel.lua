...
function playerMeta:GetMaxHealth(health)
	...
	if factionName == "Order of Eternal Penance" then
	...
	if subfaction then
		...
		elseif subfaction == "Clan Reaver" or subfaction == "Clan Crast" or subfaction == "Legionary" or subfaction == "Varazdat" or subfaction == "Shepherds of Light" then
			maxHealth = maxHealth + 25;
		elseif subfaction == "Aswaran" then
			maxHealth = maxHealth + 45;
		elseif subfaction == "Immortal" then
			maxHealth = maxHealth + 300;
		elseif subfaction == "Yihnari" then
			--[[local coin = self:GetCash(); -- fuck that shit? who coded this - sprite

			if coin >= 5000 then
				coin = 5000;
			end

			maxHealth = maxHealth + 25 + (coin/200);
			]]--
	...
	if faith == "Faith of the Many" then	
		if self:HasBelief("sows_belief1") then
			maxHealth = maxHealth + 10;
		end
		
		if self:HasBelief("sows_belief2") then
			maxHealth = maxHealth + 20;
		end

		local hunger = self:GetNeed("hunger")

		if self:HasBelief("sows_ritual3") and hunger < 0 then
			maxHealth = maxHealth + -hunger;
		end
	end
	...
	local gang = self:GetCharacterData("scrapperGang");

	if gang then
		if Schema.scrapperGangs[gang] then 
			if (Schema.scrapperGangs[gang].memberCount >= 8 or (Schema.scrapperGangs[gang].memberCount >= 7 and Schema.scrapperGangs[gang].leader)) then
				maxHealth = maxHealth + 15;
			end
		end
	end
