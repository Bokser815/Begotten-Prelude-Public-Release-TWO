...
function PANEL:Init()
	...
	function self.btnClose.DoClick(button)
		...
		netstream.Start("SalesmanAdd", {
			...
			weapon = Clockwork.salesman.weapon,
		})
	...
	self.weapon = self.settingsForm:TextEntry("The weapon class the salesman uses.");
	self.weapon:SetValue(Clockwork.salesman.weapon or "begotten_1h_ironarmingsword");
	...
	self.warnText = self.responsesForm:TextEntry("When the salesman is injured by a player.");
	self.warnText:SetValue(Clockwork.salesman.text.warn.text or "Agh!");

	self.hostileText = self.responsesForm:TextEntry("When the salesman becomes hostile to a player.");
	self.hostileText:SetValue(Clockwork.salesman.text.hostile.text or "Don't say I didn't warn you!");
...
function PANEL:Think()
	...
	Clockwork.salesman.text.warn = {
		text = self.warnText:GetValue(),
		bHideName = (self.startHideName:GetChecked() == true),
		sound = self.noSaleSound:GetValue()
	}
	Clockwork.salesman.text.hostile = {
		text = self.hostileText:GetValue(),
		bHideName = (self.startHideName:GetChecked() == true),
		sound = self.noSaleSound:GetValue()
	}
	...
	Clockwork.salesman.weapon = self.weapon:GetValue();
