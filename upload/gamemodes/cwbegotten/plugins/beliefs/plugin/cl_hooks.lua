...
function cwBeliefs:CanPlayerDualWield()
	return self:HasBelief({"man_become_beast", "murder_artform", "repentant", "witch_druid", "vultures_ritual1", "libertines_ritual1"}, true);
end
...
function cwBeliefs:AddEntityOutlines(outlines)
	...
	if Clockwork.Client:GetSharedVar("faith") == "Faith of the Many" then
		local hasTracking = self:HasBelief("vultures_belief2");

		for i, v in ipairs(_player.GetAll()) do
			if v ~= Clockwork.Client and v:HasInitialized() and v:Alive() and v:GetColor().a > 0 then
				if hasTracking and v.bleeding then
					if (v:GetPos():DistToSqr(Clockwork.Client:GetPos()) <= bleedDist) then
						self:DrawPlayerOutline(v, outlines, Color(59, 17, 25));
					end
				end
			end
		end
	...
	if self.upgradedWarcryActive then
		--[[if self.trout then
			for _, v in _player.Iterator() do
				...
				if self:HasBelief("sows_belief3") then
					if v.warcryTarget and v:Alive() and v:GetColor().a > 0 and v.hunger < 75 then
...
netstream.Hook("UpgradedWarcry", function(data)
		...
		if v ~= Clockwork.Client and (v:HasInitialized()) then
			--if v:GetNetVar("faith") ~= faith then
				...
				if faction == "Wanderer" or vFaction ~= faction then
					...
					if cwBeliefs:HasBelief("sows_belief3") then
						if (v:GetPos():DistToSqr(Clockwork.Client:GetPos()) <= (800 * 800)) and v.hunger < 75 then
							v.warcryTarget = true;
						end;
					else
	...
	-- removing new warcry highlights for now prob - sprite
	--end
end);