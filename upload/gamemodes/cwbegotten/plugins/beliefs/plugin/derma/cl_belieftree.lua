...
local factiontranslations = {
	["Clan Grock"] = "Old Knowledge"
...
function PANEL:Paint()
	...
	local altlevels = tonumber(Clockwork.Client:GetSharedVar("alternatelevels"),10)
	local translatedvalue = factiontranslations[Clockwork.Client:GetSharedVar("subfaction")]
	...
	if altlevels and isnumber(altlevels) and altlevels > 0 and translatedvalue then

		--local subfactiontype = Clockwork.Client:GetSharedVar("subfaction")
		draw.SimpleText("Faith Concentrated: "..math.floor(experience)..", "..translatedvalue..": "..math.floor(altlevels), "Civ5Tooltip1",(self.barX) + 4 + 16 + 512, ((self.barY) + 3), Color(200, 0, 0, 255))
		draw.SimpleText("Faith Concentrated: "..math.floor(experience)..", "..translatedvalue..": "..math.floor(altlevels), "Civ5Tooltip1", (self.barX) + 6 + 16 + 512, ((self.barY) + 5), Color(0, 0, 0, 127))
	elseif level < self.localLevelCap then
...
function PANEL:AddIcon(iconData)
	...
	local paintingicon = Material("begotten/ui/penor.png")
	...
	function icon.DoClick()
		...
		if iconData.cost and iconData.cost > Clockwork.Client:GetSharedVar("alternatelevels") then
			...
			if (beliefs[icon.uniqueID]) then
				...
				if canUnlock then
					...
					elseif icon.subfaith and beliefPanel.player:GetNetVar("subfaith") and beliefPanel.player:GetNetVar("subfaith") ~= "" and beliefPanel.player:GetNetVar("subfaith") ~= "N/A" and icon.subfaith ~= beliefPanel.player:GetNetVar("subfaith") then
						...
						canTakeColor = selectedBad;
					elseif iconData.cost and iconData.cost > Clockwork.Client:GetSharedVar("alternatelevels") then
						local translatedvalue = factiontranslations[Clockwork.Client:GetSharedVar("subfaction")]
						canTake = "\nYou need "..iconData.cost-Clockwork.Client:GetSharedVar("alternatelevels").." more "..translatedvalue.." for this belief."
						canTakeColor = selectedBad;
	...
	icon.Paint = function( self, w, h )
		if iconData.cost and (parent.locked != true and !icon.locked and !icon.disabled) and ((iconData.subfaith==subfaith) or subfaith=="N/A") then
			local percentage =  Clockwork.Client:GetSharedVar("alternatelevels")/iconData.cost

			draw.RoundedBox( 0, 0, 0, w, h, Color(0,0,0,0) )
			surface.SetMaterial(paintingicon)
			surface.SetDrawColor(Color(255,0,0,255))
			surface.DrawTexturedRectUV( 0, 0+(h*percentage), w, h, 0, 0, 1, 1 )
		end
		if iconData.cost and (parent.locked != true and !icon.locked and !icon.disabled) and ((iconData.subfaith==subfaith) or subfaith=="N/A") then
