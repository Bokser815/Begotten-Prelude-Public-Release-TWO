...
local COMMAND = Clockwork.command:New("CharSetAlternateLevel");
COMMAND.tip = "Set a player's alternate Level xp.";
COMMAND.text = "<string Name> <int AlternateXP>";
COMMAND.flags = CMD_DEFAULT;
COMMAND.access = "s";
COMMAND.arguments = 2;
COMMAND.alias = {"SetaltLevel", "PlySetaltLevel"};
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		...
		if level then
			...
			if level > 0 then
				if (player != target) then
					Schema:EasyText(player, "cornflowerblue", "["..self.name.."] You have set "..target:Name().."'s alternate level to "..tostring(level)..".");
				else
					Schema:EasyText(player, "cornflowerblue", "["..self.name.."] You have set your own alternate level to "..tostring(level)..".");
				end;
				--cwBeliefs:SetSacramentLevel(target, level);
				target:SetCharacterData("alternatelevels", level)
				target:SetSharedVar("alternatelevels",level)
			else
...
local COMMAND = Clockwork.command:New("Warcry");
	...
	function COMMAND:OnRun(player, arguments)
		...
		local player_has_markofregression = player:HasBelief("regressiongrock");
		local player_has_libertines_belief3 = player:HasBelief("libertines_belief3")
		local player_has_sows_belief3 = player:HasBelief("sows_belief3")
		...
		if player:GetNetVar("kinisgerOverride") then
			...
			elseif faith == "Old Ways" then
				sanity_debuff = -15;
				warcry_beliefs = {"bludgeongrock"};
			elseif faith == "Faith of the Dark" then
				sanity_debuff = -25;
				warcry_beliefs = {"sadism"};
			elseif faith == "Faith of the Many" then
				sanity_debuff = -15;
				warcry_beliefs = {"libertines_belief2", "sows_belief2"}
			...
			if (!player.lastWarCry) or player.lastWarCry < curTime then
				...
				for k, v in pairs(ents.FindInSphere(playerPos, radius)) do
					...
					if (isPlayer and v:GetMoveType() == MOVETYPE_WALK) then
						...
						local vCov = v:GetNetVar("wandererCovenantName")
						local pCov = player:GetNetVar("wandererCovenantName")

						if pCov and vCov == pCov then
							immune = true
							break
						end
						...
						--if v:GetFaith() ~= faith then
							...
							elseif faith == "Faith of the Family" then
								...
								if !immune then
									if Schema.towerSafeZoneEnabled or !v:InTower() then
										...
										if player_has_fearsome_wolf or player_has_daring_trout or player_has_markofregression then
							...
							elseif faith == "Faith of the Many" then
								if (faction == "Darklanders" and vFaction == "Darklanders") then
									immune = true;
								elseif v.banners then
									for k2, v2 in pairs(v.banners) do
										if v2 == "glazic" then
							...
							if player_has_sows_belief3 then -- For thirty seconds any damage you deal is returned to your stability.
								player.stupidFuckingPigWantsToStayUp = true 
								timer.Simple(30, function()
									if IsValid(player) then 
										player.stupidFuckingPigWantsToStayUp = false
									end
								end)
							end
						--end
				...
				elseif faith == "Faith of the Many" then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "me", "lets out a hollering screech!", playerPos, radius);
				else
					player:HandleSanity(-5);
				...
				if player_has_fearsome_wolf or player_has_daring_trout or player_has_libertines_belief3 or player_has_sows_belief3 then
					...
					if player_has_daring_trout then
          			...
          			elseif player_has_markofregression then

						if IsValid(player) and player.warCryVictims then
							local savedvictimtable = player.warCryVictim
							
							for i = 1, #savedvictimtable do
								local victim = savedvictimtable[i];
								
								if IsValid(victim) then
									victim.markofregression = true
								end
							end

							timer.Simple(20.5, function()
								for i = 1, #savedvictimtable do
									local victimception = savedvictimtable[i];
									if IsValid(victimception) then
										victimception.markofregression = nil;
									end
								end
							end);
						...
						timer.Simple(20.5, function()
							if IsValid(player) then
								player.regressionspeed = nil;
								
								hook.Run("RunModifyPlayerSpeed", player, player.cwInfoTable, true);
							end
						end);
					elseif player_has_fearsome_wolf then
					...
					elseif player_has_libertines_belief3 then
						timer.Simple(10.5, function()
							if IsValid(player) then
								player.warCryVictims = nil;
							end
						end);
					elseif player_has_sows_belief3 then
						timer.Simple(10.5, function()
							if IsValid(player) and player.warCryVictims then
								-- its just supposed to highlight them
...
function COMMAND:OnRun(player, arguments)
	...
	if weapon.category == "Firearms" and raised and not player:GetSharedVar("soulscorchActive") then
		player:CommitSuicideGun();
	elseif player:HasBelief("sol_orthodoxy") or player:HasBelief("vultures_ritual1") then
