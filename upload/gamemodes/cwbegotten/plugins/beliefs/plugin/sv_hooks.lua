	...
	if initialized and alive then
		if (!plyTab.residualXPCheck or plyTab.residualXPCheck < curTime) then
			...
			if not plyTab.opponent and (table.HasValue(self.residualXPZones, lastZone) or (lastZone == "tower" and ((playerFaction == "Gatekeeper" or playerFaction == "Pope Adyssa's Gatekeepers" or playerFaction == "Hillkeeper" or playerFaction == "Holy Hierarchy") or residualXPInSafezone == true))) then
				...
				if player:HasBelief("the_libertines") then
					local playerCoin = player:GetCash()
					local coinperXP = 300; 

					residualXP = residualXP + math.Round(coinperXP / playerCoin);
				end
		...
		if !plyTab.bucketCheck or plyTab.bucketCheck < curTime then
			...
			if (player:HasItemByID("dirty_water_bucket") or player:HasItemByID("purified_water_bucket")) and !player:HasBelief("dexterity") and !player.cwObserverMode then
		...
		if (!plyTab.regenCheck or plyTab.regenCheck < curTime) then
			...
			if player:HasBelief("sows_ritual3") and hunger < 0 then -- prob should make the hp regen rate based on how much you've overfed
				if hook.Run("PlayerShouldHealthRegenerate", player) then
					local maxHealth = player:GetMaxHealth()
					local health = player:Health()
...
function cwBeliefs:PostPlayerSpawn(player, lightSpawn, changeClass, firstSpawn)
	local altlevels = player:GetCharacterData("alternatelevels", 0)
	player:SetSharedVar("alternatelevels", altlevels)
	if player.cloaked then
		player:Uncloak();
	end
	if player:HasTrait("deaf") then player:SetDSP(31) else player:SetDSP(0) end
end;
						...
						elseif entity:GetNWEntity("Player"):IsPlayer() or entity:GetNWEntity("Player") == game.GetWorld() then
							...
							Clockwork.player:SetAction(player, "mutilating", mutilationTime, 5, function()
								if IsValid(player) and IsValid(entity) then
									...
									if activeWeapon:IsValid() and activeWeapon.isDagger or offhandWeapon and offhandWeapon.isDagger and weaponItemTable then
										if (!entity.mutilated or entity.mutilated < 3) or ((!entity.mutilated or entity.mutilated < 5) and player:HasBelief("vultures_ritual3")) then
											entity.mutilated = (entity.mutilated or 0) + mutilationValue;
											
											local instance = Clockwork.item:CreateInstance("humanmeat");

											player:GiveItem(instance, true);
											if player:HasBelief("sows_ritual2") then
				...
				if entity:GetNWEntity("Player"):IsPlayer() or entity:GetNWEntity("Player") == game.GetWorld() or table.HasValue(animalModels, model) then
					if (!entity.hearteaten) then
						...
						if player.masticationActive then
							...
							player:ModifyBloodLevel(666);

							player:ScreenFade(SCREENFADE.OUT, Color(100, 20, 20, 80), 0.2, 0.1);
						
							timer.Simple(0.2, function()
								if IsValid(attacker) then
									player:ScreenFade(SCREENFADE.IN, Color(100, 20, 20, 80), 0.2, 0);
								end
							end);
						end
				...
				if attackerWeapon.Base == "sword_swepbase" then -- Melee
					if attacker:GetCharmEquipped() then
						if attackerWeapon:GetClass() == "begotten_fists" then
							if IsValid(damageInfo:GetInflictor()) and damageInfo:GetInflictor().isJavelin then
								...
								if attacker:GetFaith() == "Old Ways" and !entity:IsPlayer() then
									newDamage = newDamage + (originalDamage * 5);
								end
					...
					if attackerWeapon:GetClass() == "begotten_fists" then
						if IsValid(damageInfo:GetInflictor()) and damageInfo:GetInflictor().isJavelin then
							...
							if attacker:GetFaith() == "Old Ways" and !entity:IsPlayer() then
								newDamage = newDamage + (originalDamage * 5);
								if !entity.fistdamage then entity.fistdamage = 0 end
								entity.fistdamage = entity.fistdamage+newDamage
							end
					...
					if attacker:HasBelief("vultures_belief1") then
						local attackTable = GetTable(attackerWeapon.AttackTable);
					...
					local intoxication = attacker:GetNeed("intoxication") -- really this drunk shit shouldn't be here but i'm lazy - sprite

					if intoxication >= 10 then
						if string.find(attackerWeapon.Category, "Fisted") then
							newDamage = math.floor(newDamage * Lerp(intoxication / 100, 1, 1.5))
						end
					end

					if (entity:IsPlayer()) then
						local hunger = entity:GetNeed("hunger")

						if attacker:HasBelief("vultures_belief3") and entity.bleeding then
							local bleedingLimbsData = entity:GetCharacterData("BleedingLimbs", {});
							local bleedingLimbsCounter = 0
						
							for k, v in pairs(bleedingLimbsData) do
								if v == true then
									bleedingLimbsCounter = bleedingLimbsCounter + 1;
								end
							end
							
							newDamage = newDamage + (originalDamage * (0.025 * bleedingLimbsCounter)); -- note for sprite, test this
						end

						if attacker:HasBelief("sows_belief1") and hunger > 25 then
							newDamage = newDamage + (originalDamage * 0.10);
						end
					end
--[[
					if attacker:HasBelief("sows_belief2") then
						if string.find(attackerWeapon.Category, "Two Handed") or string.find(attackerWeapon.Category, "Great Weapon") or string.find(attackerWeapon.Category, "Scythe") then
							newDamage = newDamage + (originalDamage * 0.10);
						end
					end
]]--
					if attacker:HasBelief("libertines_belief1") and entity:IsPlayer() and entity:GetCash() < attacker:GetCash() then
						newDamage = newDamage + (originalDamage * 0.10);
					end
			...
			if entity:IsPlayer() then
				...
				local playerCash = attacker:GetCash();
				local entityCash = entity:GetCash();

				if attacker:HasBelief("libertines_ritual2") then
					if playerCash < entityCash then
						newDamage = newDamage + (originalDamage * 0.10);
					end
				end

				if attacker:HasBelief("libertines_ritual3") then
					if (attacker:GetGender() == GENDER_MALE and entity:GetGender() == GENDER_FEMALE) or (attacker:GetGender() == GENDER_FEMALE and entity:GetGender() == GENDER_MALE) then
						newDamage = newDamage + (originalDamage * 0.15);
					end
				end
				...
				if !cwRituals or (cwRituals and !entTab.scornificationismActive) then
					...
					if attackerFactionTable then
						...
						if (attackerFaction ~= playerFaction and (!attackerFactionTable.alliedfactions or !table.HasValue(attackerFactionTable.alliedfactions, playerFaction))) or attackerFaction == "Wanderer" then
							...
							if attacker:HasBelief("sister") then
								if attacker:GetCharacterData("level", 1) > entity:GetCharacterData("level", 1) then
									damageXP = damageXP * 2;
								end
							end
...
function cwBeliefs:PrePlayerCharacterCreated(player, character)
	...
	elseif faction == "Goreic Warrior" then
		...
		elseif subfaction == "Clan Grock" then
			level = level + 7;
				player:SetCharacterData("faith", "Old Ways",true) -- TODO: idk why det changed it to this but prob micro optimization
				player:SetSharedVar("faith","Old Ways")
				player:SetCharacterData("alternatelevels", 1)
				player:SetSharedVar("alternatelevels",1)
		end
	...
	elseif faction == "The Third Inquisition" then
			level = level + 19;
	elseif faction == "Darklanders" then
		if subfaction == "Yihnari"  then 
			level = level + 8;
		elseif subfaction == "Nehemian" then
			level = level + 10;
			data["beliefs"]["ingenious"] = true;
			data["beliefs"]["powder_and_stee"] = true;
			data["beliefs"]["pistolier"] = true;
			data["beliefs"]["marksman"] = true;
		elseif subfaction == "Aswaran" then 
			level = level + 9;
		elseif subfaction == "Immortal" then
			level = level + 35;
			data["beliefs"]["fighter"] = true;
			data["beliefs"]["strength"] = true;
			data["beliefs"]["defender"] = true;
			data["beliefs"]["warden"] = true;
			data["beliefs"]["hauberk"] = true;
			data["beliefs"]["savage"] = true;
			data["beliefs"]["heart_eaters"] = true;
		end
	elseif faction == "Order of Eternal Penance" then
			level = level + 8;
			data["beliefs"]["sol_orthodoxy"] = true;
	...
	if player:HasTrait("wanted") then
		local coin = math.random(20, 50);
		local reasons = {
			"Thievery",
			"Mugging",
			"Complaining",
		...
		local hereticreasons = {
			"Apostasy",
			"Paganism",
			"Heresy",
		}

		if player:GetFaith() ~= "Faith of the Light" then
			table.Add(reasons, hereticreasons)
		end
		if player:HasTrait("cannibal") then
			table.Insert(reasons, "Cannibalism")
		end

		local chosenreason = table.Random(reasons)

		player:AddBounty(coin, chosenreason, "The Holy Hierarchy");

		Schema:EasyText(player, "maroon", "You are wanted by the Holy Hierarchy for '"..chosenreason.."'.")
	end
...
function cwBeliefs:PlayerDeath(player, inflictor, attacker, damageInfo)
	if IsValid(attacker) and attacker:IsPlayer() and not player.opponent and not attacker.opponent then
		if attacker:HasBelief("brutality_finisher") then
			...
			if attacker:HasBelief("brutalgrock") then -- TODO: bokser u a genius, i need to rebalance this later - sprite
				attacker:ResetInjuries();
				attacker:TakeAllDiseases();
				attacker:SetNWInt("freeze", 0);
				attacker:SetBloodLevel(5000);
				attacker:StopAllBleeding();
				attacker:Extinguish()
				Clockwork.limb:HealBody(attacker, 100);
			end
		...
		if player:CharPlayTime() > config.GetVal("min_xp_charplaytime") then
			...
			if attackerFactionTable then
				...
				if (attackerFaction ~= playerFaction and (!attackerFactionTable.alliedfactions or !table.HasValue(attackerFactionTable.alliedfactions, playerFaction))) or attackerFaction == "Wanderer" then
					...
					local attackerCash = attacker:GetCash();
					...
					if attacker:HasBelief("libertines_ritual3") then
						if (attacker:GetGender() == GENDER_MALE and player:GetGender() == GENDER_FEMALE) or (attacker:GetGender() == GENDER_FEMALE and player:GetGender() == GENDER_MALE) then
							killXP = killXP * 2;
						end
					end
					if attacker:HasBelief("libertines_ritual2") then
						if attackerCash < playerCash then
							killXP = killXP * 2;
						end
					end
					if attacker:HasBelief("the_vultures") then
	...
	if player:HasTrait("deaf") then player:SetDSP(0) end
	...
	if gang then
		if Schema and Schema.scrapperGangs then
			if Schema.scrapperGangs[gang].memberCount == 1 then
				...
				Clockwork.player:NotifyAdmins("operator", "The "..gang.." Scrapper Gang has been defeated.");
			else
...
function cwBeliefs:ModifyPlayerSpeed(player, infoTable)
	...
	if player:HasBelief("bulwarkgrock") then
		infoTable.runSpeed = infoTable.runSpeed * 0.95;
	end -- sometimes you just gotta give in to yandere dev logic
	...
	if player:HasBelief("purity_afloat") then
		--if not clothesItem or (clothesItem and (clothesItem.weightclass ~= "Heavy" or player:GetFaction() == "Order of Eternal Penance")) then
	...
	if player:HasBelief("sows_ritual3") and hunger < 0 then
		infoTable.runSpeed = infoTable.runSpeed * Lerp(-hunger / 100, 0.6, 1)
		infoTable.walkSpeed = infoTable.walkSpeed * Lerp(-hunger / 100, 0.7, 1)
	end
end
