...
cwBeliefs.xpValues = {
	...
	["grockwound"] = 50, -- when a grock breaks a bone
};
...
cwBeliefs.beliefNotifications = {
	...
	["the_libertines"] = "You can now pray to the Gods by typing '/pray' in the chatbox.",
	["the_vultures"] = "You can now pray to the Gods by typing '/pray' in the chatbox.",
	["the_sows"] = "You can now pray to the Gods by typing '/pray' in the chatbox.",
	...
	["libertines_ritual1"] = "You can now dual-wield weapons by combining them in the inventory tab. Only one-handed weapons and daggers can be dual-wielded. You will only be able to thrust attack if both weapons are capable of alternate attacks.",
	["vultures_ritual1"] = "You can now dual-wield weapons by combining them in the inventory tab. Only one-handed weapons and daggers can be dual-wielded. You will only be able to thrust attack if both weapons are capable of alternate attacks. You can also now end your miserable fucking existence by typing '/suicide'.",
};
...
function cwBeliefs:PlayerPickupCustom(player, entity)
	if IsValid(player) and IsValid(entity) then
		local faith = player:GetFaith()
		if faith and faith == "Old Ways" then
			local tainted = entity.istaintedbyweak
			if tainted == true then
				--lose xp
				local altlevels = player:GetCharacterData("alternatelevels", 0)
				if altlevels > 0 then
					if altlevels<10 then
						local level = 0
						player:SetCharacterData("alternatelevels", level)
						player:SetSharedVar("alternatelevels",level)
					else
						local level = altlevels-10
						player:SetCharacterData("alternatelevels", level)
						player:SetSharedVar("alternatelevels",level)
					end
					Clockwork.hint:Send(player, "You feel disgusting...", 5, Color(175, 100, 100));
				end
				--player:EmitSound("vulturenoise1.wav")
...
function cwBeliefs:PlayerDropCustom(player, entity)
	if IsValid(player) and IsValid(entity) then
		local faith = player:GetFaith()
		if faith and faith == "Old Ways" then
			entity.istaintedbyweak = false
		else
			entity.istaintedbyweak = true
		end
	end
end

function cwBeliefs:PlayerTakeFromContainerCustom(player, entity)
	if IsValid(player) and IsValid(entity) then
		local faith = player:GetFaith()
		if faith and faith == "Old Ways" then
			local tainted = entity.istaintedbyweak
			if (tainted and tainted == true) or entity:GetModel()!="models/props_clutter/barrel02.mdl" then
				local altlevels = player:GetCharacterData("alternatelevels", 0)
				if altlevels>0 then
					if altlevels<25 then
						local level = 0
						player:SetCharacterData("alternatelevels", level)
						player:SetSharedVar("alternatelevels",level)
					else
						local level = altlevels-25
						player:SetCharacterData("alternatelevels", level)
						player:SetSharedVar("alternatelevels",level)
					end
					Clockwork.hint:Send(player, "You feel disgusting...", 5, Color(175, 100, 100));
...
function cwBeliefs:PlayerGiveToContainerCustom(player, entity)
	if IsValid(player) and IsValid(entity) then
		local faith = player:GetFaith()
		if faith and faith == "Old Ways" then
		else
...
function cwBeliefs:TakeBelief(player, uniqueID, niceName, category)
	...
	local serversidealtlevelcount = player:GetCharacterData("alternatelevels", 0)
	local allegedaltxp = player:GetSharedVar("alternatelevels")
	if !allegedaltxp then allegedaltxp = 0 end
	
	if (takeBelief and !beliefs[uniqueID]) and (allegedaltxp == serversidealtlevelcount) then
		...
		if category == "old ways" then
			player:SendLua([[Clockwork.Client:EmitSound("fiend/specialbelief.wav", 40)]]);
		else
	...
	elseif (allegedaltxp != serversidealtlevelcount) then
		player:SetSharedVar("alternatelevels",serversidealtlevelcount)
		player:Notify("Values synced once again!")
	end;
end;
