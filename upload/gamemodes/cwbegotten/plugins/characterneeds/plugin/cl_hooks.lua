...
function cwCharacterNeeds:ModifyStatusEffects(tab)
	...
	local corruption = tonumber(Clockwork.Client:GetNetVar("corruption"));
	local sleep = tonumber(Clockwork.Client:GetNetVar("sleep"));
	local intoxication = tonumber(Clockwork.Client:GetNetVar("intoxication"));
	...
	if hunger <= -50 then
		table.insert(tab, {text = "(+) Overfed", color = Color(0, 225, 0)});
	end
	...
	if intoxication >= 75 then
		table.insert(tab, {text = "(-) Intoxicated", color = Color(200, 40, 40)});
	end
...
function cwCharacterNeeds:RenderScreenspaceEffects()
	if Clockwork.Client:HasInitialized() then
		...
		local intoxication = tonumber(Clockwork.Client:GetNetVar("intoxication", 0));
