...
function COMMAND:OnRun(player, arguments)
	...
	if (player:GetNeed("sleep") >= 40) then
		if cwShacks and cwShacks.shacks then
			for k, v in pairs(cwShacks.shacks) do
				if player:GetPos():WithinAABox(v.pos1, v.pos2) then
					if v.bedTier >= 2 then
						player.sleepData = {health = 50, hunger = 5, thirst = 10, rest = -100, sanity = 50, intoxication = -33};
					else
						player.sleepData = {health = 25, hunger = 10, thirst = 20, rest = -60, sanity = 25, intoxication = -20};
					end
			...
			--elseif faction == "Holy Hierarchy" then
				...
				if cwCharacterNeeds.bedZones["ministers2"] then
					if playerPos:WithinAABox(cwCharacterNeeds.bedZones["ministers2"].pos1, cwCharacterNeeds.bedZones["ministers2"].pos2) then
						player.sleepData = {health = 200, hunger = 1, thirst = 1, rest = -100, sanity = 100};
						--player:HandleSanity(50);
						--player:HandleNeed("hunger", 5);
						--player:HandleNeed("thirst", 10);
						--player:HandleNeed("sleep", -100);
						...
						Clockwork.player:SetRagdollState(player, RAGDOLL_KNOCKEDOUT, 100);
						Schema:EasyText(player, "olivedrab", "You climb into the pope's bed and have a heavenly rest. You feel refreshed beyond measure.");
						return;
					end
				end
		...
		--elseif faction == "Smog City Pirate" then
			...
			if cwCharacterNeeds.bedZones["scrapper2"] then
				if playerPos:WithinAABox(cwCharacterNeeds.bedZones["scrapper2"].pos1, cwCharacterNeeds.bedZones["scrapper2"].pos2) then
					player.sleepData = {health = 40, hunger = 10, thirst = 15, rest = -100, sanity = 40, intoxication = -33};
					--player:HandleSanity(50);
					--player:HandleNeed("hunger", 5);
					--player:HandleNeed("thirst", 10);
					--player:HandleNeed("sleep", -100);
					...
					Clockwork.player:SetRagdollState(player, RAGDOLL_KNOCKEDOUT, 100);
					Schema:EasyText(player, "olivedrab", "You climb into a dirty slave cot and get some rest.");
					return;
				end
			end
		...
		player.sleepData = {health = 10, hunger = 15, thirst = 30, rest = -30, intoxication = -10};
...
local COMMAND = Clockwork.command:New("CharSetIntoxication");
COMMAND.tip = "Set a player's Intoxication level. (0 = Sober, 100 = Smashed)";
COMMAND.text = "<string Name> <number Amount>";
COMMAND.access = "o";
COMMAND.arguments = 2;
COMMAND.alias = {"SetIntoxication", "PlySetIntoxication", "SetDrunk", "CharSetDrunk", "PlySetDrunk"};
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		target:SetNeed("intoxication", amount);
		
		if (player != target) then
			Schema:EasyText(GetAdmins(), "cornflowerblue", "["..self.name.."] "..player:Name().." has set "..target:Name().."'s intoxication to "..amount..".");
		else
			Schema:EasyText(player, "cornflowerblue", "["..self.name.."] You have set your own intoxication to "..amount..".");
		end;
	else
		Schema:EasyText(player, "grey", "["..self.name.."] "..arguments[1].." is not a valid player!");
	end;
...
local COMMAND = Clockwork.command:New("CharGetNeeds");
	...
	function COMMAND:OnRun(player, arguments)
		...
		if (target and target:HasInitialized() and target:Alive()) then
			...
			local intoxication = target:GetNeed("intoxication", 0);
			local needsStr = "Hunger: "..hunger.."  Thirst: "..thirst.."  Fatigue: "..fatigue.."  Corruption: "..corruption.."  Intoxication: "..intoxication;
