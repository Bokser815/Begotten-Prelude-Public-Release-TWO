...
function cwCharacterNeeds:PlayerThink(player, curTime, infoTable, alive, initialized, plyTab)
	if !plyTab.nextNeedCheck or curTime >= plyTab.nextNeedCheck then
		...
		if (!plyTab.nextHunger or curTime >= plyTab.nextHunger) then
			...
			if(player:HasTrait("giant")) then
				next_hunger = next_hunger * 0.7;
			end

			if player:HasBelief("sows_belief3") then
				local hunger = player:GetNeed("hunger")
				if hunger < 0 then
					next_hunger = next_hunger * Lerp(-hunger / 100, 0.35, 0.0025)
				else
					next_hunger = next_hunger * 0.35;
				end
			end
			...
			elseif (playerNeeds["hunger"] > -101 and player:HasBelief("sows_ritual3")) then
				player:HandleNeed("hunger", 1);
			end;
		end;
		...
		if (!plyTab.nextIntoxication or curTime >= plyTab.nextIntoxication) then
			local next_intoxication = 100;
			--[[
			if player:HasTrait("gluttony") then
				next_thirst = next_thirst * 0.35;
			end
			...
			if player:HasBelief("asceticism") then
				next_intoxication = next_intoxication * 1.35;
			end
			
			plyTab.nextIntoxication = curTime + next_intoxication;
		
			if (playerNeeds["intoxication"] > -1) then
				player:HandleNeed("intoxication", -1);
			end
		end;
	...
	if itemTable.needs then
		...
		for k, v in pairs(itemTable.needs) do
			...
			elseif subfaction ~= "Varazdat" then
				if player:HasBelief("vultures_belief1") then
					if itemTable.uniqueID == "humanmeat" or itemTable.uniqueID == "cooked_human_meat" or itemTable.uniqueID == "canned_fresh_meat" or itemTable.uniqueID == "varazdat_bloodwine" or itemTable.uniqueID == "varazdat_masterclass_bloodwine" then
						v = v * 1.5
						player:HandleNeed(k, -v);
					end
		...
		if player:HasBelief("the_sows") then
			player:HandleXP(cwBeliefs.xpValues["food"] * 2); -- just run the function again lol
		end
	end
end;
...
function cwCharacterNeeds:ActionCompletedPreCallback(player, action)
	if action == "unragdoll" and player.sleepData then
		...
		if sleepData.intoxication then
			player:HandleNeed("intoxication", sleepData.intoxication);
		end
