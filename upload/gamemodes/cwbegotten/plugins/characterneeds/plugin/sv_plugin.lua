...
cwCharacterNeeds.Needs = {"corruption", "hunger", "thirst", "sleep", "intoxication"};
...
elseif (game.GetMap() == "bg_prelude") then
	cwCharacterNeeds.bedZones = {
		["gatekeeper"] = {pos1 = Vector(438,11677,4587), pos2 = Vector(926,11892,4827)},
		["hierarchy"] = {pos1 = Vector(2515,12099,4444), pos2 = Vector(2191,12955,4857)},
		["ministers"] = {pos1 = Vector(3468,13229,4536), pos2 = Vector(3123,14090,4871)},
		["ministers2"] = {pos1 = Vector(1025,15015,6219), pos2 = Vector(859,14874,6402)},
		["castle"] = {pos1 = Vector(-12837,-13296,4426), pos2 = Vector(-12481,-12759,4613)},
		["scrapper1"] = {pos1 = Vector(-10463,13260,-273), pos2 = Vector(-9790,12918,-49)},
		["scrapper2"] = {pos1 = Vector(-8641,12184,-650), pos2 = Vector(-10567,12003,-16)},
		--gore
		--castle?
		--scrapperboat?
		--poolroom??
...
function playerMeta:HandleNeed(need, amount)
	...
	if need and table.HasValue(cwCharacterNeeds.Needs, need) then
		...
		if amount > 0 then
			if need == "sleep" then
				if cwBeliefs and self:HasBelief("enduring_bear") then
					amount = (amount / 2);
				elseif self:HasBelief("shrewdgrock") then
					amount = (amount / 1.5);
				end
							...
							if IsValid(self) then
								...
								if self:Alive() then
									...
									if lastZone ~= "tower" and lastZone ~= "theather" and lastZone ~= "manor" then 
										...
										Schema:Summon(playerPos, Angle(0,0,1), function()
		...
		if(need == "hunger" and self:HasBelief("sows_ritual3")) then -- overfeeding
			self:SetCharacterData(need, math.Clamp(newAmount, -100, 100));
			self:SetSharedVar(need, math.Clamp(math.Round(newAmount), -100, 100));
