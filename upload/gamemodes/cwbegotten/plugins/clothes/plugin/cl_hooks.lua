		...
		if !v:IsDormant() and string.sub(v:GetModel(), 1, 21) == "models/begotten/heads" then
			...
			if !IsValid(clothesEnt) then
				...
				if IsValid(clothesEnt) then
					...
					clothesEnt:SetRenderMode(RENDERMODE_TRANSCOLOR);
...
function PLUGIN:PostPlayerDraw(player, flags)
	if string.sub(player:GetModel(), 1, 21) == "models/begotten/heads" then
		...
		if player:Alive() and player:GetMoveType() ~= MOVETYPE_OBSERVER and plyColor.a > 0 and !player:GetNoDraw() then
			...
			if player:GetCharmEquipped("ansible_new") then
				local boneIndex = player:LookupBone("ValveBiped.Bip01_Head1")
				if boneIndex then
					local pos, angle = player:GetBonePosition(boneIndex)
					local drawData = { position = pos, angle = angle }
					drawData.scale = 0.1
					
					angle:RotateAroundAxis(-angle:Right(), 90)
					angle:RotateAroundAxis(angle:Up(), 270)
					pos:Add((angle:Up() * 60) * drawData.scale)
					
					local mdlEnt = ClientsideModel("models/props_rooftop/roof_dish001.mdl", RENDERGROUP_BOTH)
					if IsValid(mdlEnt) then
						mdlEnt:SetPos(drawData.position)
						mdlEnt:SetAngles(drawData.angle)
						mdlEnt:SetModelScale(drawData.scale, 0)
						mdlEnt:DrawModel()
						mdlEnt:Remove()
					end
				end
