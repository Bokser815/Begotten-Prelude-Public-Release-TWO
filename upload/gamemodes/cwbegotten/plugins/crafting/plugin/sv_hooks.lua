...
function cwRecipes:AddPileSpawn(pileEntity, pileType, player)
	if !IsValid(pileEntity) or (pileEntity:GetClass() ~= "cw_woodpile" and pileEntity:GetClass() ~= "cw_ironorepile" and pileEntity:GetClass() ~= "cw_scrappile") then
	...
	if pileType == "ore" or pileType == "wood" or pileType == "gorewood" or pileType == "scrap" then
...
function cwRecipes:PlayerCanCraft(player, uniqueID, craftAmount)
	...
	for k, v in pairs(requirements) do
		...
		if player:GetSubfaction() == "Greaser" and v.scrapperAmount then
			amount = v.scrapperAmount;
		end

		amount = amount * craftAmount;
		
		if (!Clockwork.inventory:HasItemCountByID(inventory, k, amount)) then
			if v.substitute then
				local count = 0;
				if (!Clockwork.inventory:HasItemCountByID(inventory, v.substitute, amount)) then
					for k2,v2 in pairs(inventory) do
						for k3, v3 in pairs(k2) do
							if v3.uniqueID == k or v3.uniqueID == v.substitute then
								count = count + 1;
							end
						end
					end

					if not count == amount then
...
function cwRecipes:PlayerFinishedCrafting(player, recipeTable, craftAmount)
	if recipeTable.experience then
		...
		elseif player:HasBelief("shrewdgrock") then
			player:HandleXP((recipeTable.experience * craftAmount) * 1.5);
		else
...
function cwRecipes:PlayerMeetsCraftingItemRequirements(player, recipeTable, itemIDs, bTake, craftAmount)
	...
	if bTake then
		local issagegrock = false
		local isfavoredgrock = false
		local chancenouse = 0
		if cwBeliefs and player:HasBelief("sagegrock") then
			issagegrock = true
		...
		for i = 1, #slottedItems do
			if isfavoredgrock == true then
				local chancenouse = math.random(1, 15)
			else
				local chancenouse = math.random(1, 20)
			end
			if issagegrock == true and (chancenouse==1) then
				-- nothing is used
			else
...
function cwRecipes:Think()
	...
	if (!self.nextPileCheck or self.nextPileCheck < curTime) then
		...
		if self.Piles then
			local categories = {"gorewood", "ore", "wood", "scrap"};
				...
				if self.pileLocations[category] then
					...
					if #unoccupiedLocations > 0 then
						...
						if self:IsAreaClear(unoccupiedLocation.pos, false) then
							...
							elseif category == "scrap" then
								pile = ents.Create("cw_scrappile")
							end
