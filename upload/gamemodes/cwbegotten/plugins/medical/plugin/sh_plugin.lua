			...
			if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
				...
				if target:Alive() then
					...
					for i = 1, #symptoms do
						...
						elseif symptom == "Laughter" then
							if symptomText then
								symptomText = symptomText.." They look to be a laughing a great deal. Clearly they've gone mad.";
							else
								symptomText = "They look to be a laughing a great deal. Clearly they've gone mad.";
							end
						end
...
local COMMAND = Clockwork.command:New("Take Blood Sample");
COMMAND.tip = "Take a sample of blood from the player before you.";
COMMAND.flags = CMD_DEFAULT;
COMMAND.alias = {"TakeBloodSample", "TakeBlood", "tbs"};

-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	local target = Clockwork.entity:GetPlayer(player:GetEyeTraceNoCursor().Entity);
	if IsValid(target) and target.bleeding and player:HasItemByID("empty_vial") then
		Schema:EasyText(player, "olivedrab", "You scoop fresh blood seeping from their wounds into your vial.");
		local empty_vial = player:FindItemByID("empty_vial");
		player:TakeItem(empty_vial);
		if target:GetFaith() ~= "Faith of the Dark" and target:GetFaith() ~= "Faith of the Many" then
			print("give vial of blood because not fotd");
			player:GiveItem(Clockwork.item:CreateInstance("vial_of_blood"));
		else
			print("give vial of tainted blood because fotd");
			player:GiveItem(Clockwork.item:CreateInstance("vial_of_tainted_blood"));
		end
	elseif IsValid(target) and target.bleeding and !player:HasItemByID("empty_vial") then
		Clockwork.player:Notify(player, "You do not have an empty vial to collect blood in!");
	else
		Clockwork.player:Notify(player, "You are not looking at a valid bleeding character!");
	end
end;

COMMAND:Register();

local COMMAND = Clockwork.command:New("Take Self Blood Sample");
COMMAND.tip = "Take a sample of blood from yourself.";
COMMAND.text = "";
COMMAND.flags = CMD_DEFAULT;
COMMAND.alias = {"TakeSelfBloodSample", "TakeOwnBlood", "tobs"};

-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	if IsValid(player) and player.bleeding and player:HasItemByID("empty_vial") then
		Schema:EasyText(player, "olivedrab", "You scoop your own fresh blood seeping from your wounds into your vial.");
		local empty_vial = player:FindItemByID("empty_vial");
		player:TakeItem(empty_vial);
		if player:GetFaith() ~= "Faith of the Dark" and player:GetFaith() ~= "Faith of the Many" then
			player:GiveItem(Clockwork.item:CreateInstance("vial_of_blood"));
		else
			player:GiveItem(Clockwork.item:CreateInstance("vial_of_tainted_blood"));
		end
	elseif IsValid(player) and player.bleeding and !player:HasItemByID("empty_vial") then
		Clockwork.player:Notify(player, "You do not have an empty vial to collect your blood in!");
	else
		Clockwork.player:Notify(player, "You are not bleeding!");
	end
end;
