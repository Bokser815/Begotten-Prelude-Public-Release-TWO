...
function cwMedicalSystem:PlayerCharacterLoaded(player)
	...
	elseif player:GetSubfaction() == "Scut" and not player:HasDisease("shackled") then
		player:GiveDisease("shackled");
	else
...
function cwMedicalSystem:PlayerThink(player, curTime, infoTable, alive, initialized, plyTab)
	if (alive and initialized) then
			...
			if (!plyTab.nextDiseaseCheck or curTime > plyTab.nextDiseaseCheck) then
				...
				if not plyTab.dyingOfDisease and not player:IsRagdolled() then
					...
					for i = 1, #symptoms do
						...
						if symptom == "Vomiting" or symptom == "Vomiting Blood" or symptom == "Coughing" or symptom == "Fatigue" or sympton == "Laughter" then
					...
					if not table.IsEmpty(valid_symptoms) then
						...
						elseif random_symptom == "Fatigue" then
							...
							local fatigue_strings = {"I feel tired...", "I need a nap...", "So tired...", "Feel like closing my eyes...", "Yawn..."};
							Clockwork.chatBox:Add(player, nil, "itnofake", curse_strings[math.random(1, #curse_strings)]);
							...
							timer.Simple(math.random(2, 5), function()
								if IsValid(player) and player:Alive() and not player:IsRagdolled() and not plyTab.iFrames and not plyTab.opponent then
									...
									randSleepy = math.random(-8,-16)
									
									player:HandleStamina(randSleepy)
									player:GiveStability(randSleepy * 4)
								end
							end);
						elseif random_symptom == "Laughter" then
							local strings = {"laughs hysterically!", "giggles uncontrollably.", "laughs wildly!", "chuckles.", "chortles and laughs!"}
							
							if (player:GetGender() == GENDER_FEMALE) then
								player:EmitSound("prelude/voice/female/laugh_"..math.random(1, 5)..".ogg", 65, math.random(90, 100));
							else
								player:EmitSound("prelude/voice/male/laugh_"..math.random(1, 7)..".ogg", 65, math.random(90, 100));
							end
		...
		if !plyTab.woundedStatusTick or plyTab.woundedStatusTick < curTime then
			local injuries = self:GetInjuries(player);
			local isInjured;

			for k, v in pairs(injuries) do
				for k2, v2 in pairs(v) do
					if v2 and v2 ~= "infection" and v2 ~= "minor_infection" and v2 ~= "major_infection" then
						isInjured = true;
						break;
					end
				end
			...
			if isInjured or player.bleeding then
				if !player:GetSharedVar("wounded") then
					player:SetSharedVar("wounded", true);
				end
			else
				if player:GetSharedVar("wounded") then
					player:SetSharedVar("wounded", false);
				end
			end

			plyTab.woundedStatusTick = curTime + 1;
		end

		if !plyTab.tumorsTick or plyTab.tumorsTick < curTime then
			if player:GetCharacterData("tumors") then
				local timer = player:GetCharacterData("tumorsTimer");

				if timer >= 1 then
					player:SetCharacterData("tumorsTimer", timer - 1);
				elseif timer == 0 then
					player:ScriptedDeath("Died to rapid growth of tumors.")
					player:SetCharacterData("tumorsTimer", nil);
					player:SetCharacterData("tumors", nil);
				else
					player:SetCharacterData("tumorsTimer", 600);
				end
			elseif not player:GetCharacterData("tumors") and player:GetCharacterData("tumorsTimer") then
				player:SetCharacterData("tumorsTimer", nil);
			end

			plyTab.tumorsTick = curTime + 1;
		end
...
function cwMedicalSystem:PostCalculatePlayerDamage(player, hitGroup, damageInfo)
	...
	local attackerbonusfaith = false

	if attacker:IsValid() and attacker:IsPlayer() and cwBeliefs then
		playerattacker = attacker
		if attacker:HasBelief("bludgeongrock") then
			attackerbonusfaith = true
		end
	end	
	...
	if !plyTab.opponent then
		...
		if damageType or bloodToll then
			...
			if cwBeliefs and attacker.HasBelief and attacker:HasBelief("vultures_belief3") then
				if damageType == DMG_SLASH and damage>=13 then
					if !player.lastbloodproc then player.lastbloodproc = 0 end
					if !player.bloodprocstacks or player.lastbloodproc+15 < CurTime() then player.bloodprocstacks = 0 end
					player.bloodprocstacks = player.bloodprocstacks + 1
					if player.bloodprocstacks == 2 then -- gross yea ik
						player:EmitSound(Sound("fiend/cursevulture.ogg"))
					elseif player.bloodprocstacks == 4 then
						player:EmitSound(Sound("fiend/cursevulture.ogg"))
					elseif player.bloodprocstacks == 6 then
						player.bloodprocstacks = 0
						player:ModifyBloodLevel(-800)
						--player:EmitSound("misc/splat.ogg", 60, math.random(80, 95));
						local effectdata = EffectData()
						effectdata:SetOrigin( player:EyePos() )
						effectdata:SetNormal( player:GetForward() )
						effectdata:SetMagnitude( 10 )
						effectdata:SetScale( 20 )
						effectdata:SetColor( 0 )
						effectdata:SetFlags( 3 )
						player:EmitSound(Sound("fiend/vulturebloodspray.ogg"))
						util.Effect( "bloodspray", effectdata, true, true )
					end
					player.lastbloodproc = CurTime()
				end
			end
			...
			if (damageThreshold and damage >= damageThreshold) and damageType ~= DMG_BURN then
				...
				elseif (damageType == DMG_CLUB and hitGroup < 8 and hitGroup > 3) or damageType == DMG_SLASH --[[or damageType == DMG_VEHICLE]] then
					...
					if attacker:IsPlayer() and attacker:HasBelief("strengthgrock") then
						chance = math.max(0, chance - (originalChance * 1.2));
					end
					...
					if attacker:IsPlayer() and attacker:HasBelief("libertines_belief3") and attacker.warCryVictims then
						if table.HasValue(attacker.warCryVictims, player) then
							chance = chance + 25
						end
					end
					...
					if math.random(1, 100) <= chance then
						...
						if damageType == DMG_CLUB then
							if !(injuries[hitGroup]["broken_bone"]) then
								...
								player:StartBleeding(hitGroup);
								if attackerbonusfaith==true then
									playerattacker:HandleXP(self.xpValues["grockwound"])
								end
							...
							if !(injuries[hitGroup]["minor_infection"]) and attacker:IsPlayer() and attacker:HasBelief("libertines_belief2") then
								player:AddInjury(self.cwHitGroupToString[hitGroup], "minor_infection");
							end
	...
	if itemTable.infectchance then
		if !cwBeliefs or (cwBeliefs and !player:HasBelief("savage_animal")) then
			if cwBeliefs and player:HasBelief("sanitary") then
				if (itemTable.uniqueID == "humanmeat" and (math.random(1,16) == 1 or player:HasTrait("marked"))) then
					player:SetCharacterData("kuruTagged", true)
					timer.Simple(600, function() -- lazy, permanent diseases don't progress through stages so yes you can dodge kuru by disconnecting but whatever im not refactoring diseases - sprite
						if player:GetCharacterData("kuruTagged", true) then
							player:GiveDisease("kuru")
						end
					end);
				else
				...
				if (itemTable.uniqueID == "humanmeat" and (math.random(1,8) == 1 or player:HasTrait("marked"))) then
					player:SetCharacterData("kuruTagged", true)
					timer.Simple(600, function() -- ditto
						if player:GetCharacterData("kuruTagged", true) then
							player:GiveDisease("kuru")
						end
					end);
...
function cwMedicalSystem:ModifyStaminaDrain(player, drainTab)
	local symptoms = player:GetSymptoms()

	if table.HasValue(symptoms, "Cramps") then
		drainTab.decrease = drainTab.decrease * 1.5;
	end
end