...
function cwMelee:DoMeleeHitEffects(entity, attacker, inflictor, position, originalDamage, damageInfo)
	if (IsValid(entity) and IsValid(attacker)) then
		...
		if entity:IsNPC() or entity:IsNextBot() or (entity:IsPlayer() and (!entity:GetNetVar("Guardening") or (entity:GetNetVar("Guardening") and !canblock)) and (!entity:GetNetVar("Parry") == true)) or entity.isTrainingDummy then
			...
			if(entity.isBlocking and entity.BlockEffect) then effectName = entity.BlockEffect; end

			if (effectName ~= "bleedingsplat" or damage > 0 and !entity.isBlocking) then
				local effectData = EffectData();
					effectData:SetOrigin(position);
					effectData:SetEntity(entity);
					effectData:SetAttachment(5);
...
function cwMelee:PlayerStabilityFallover(player, falloverTime, bNoBoogie, bNoText)
	...
	if (!Clockwork.player:HasFlags(player, "M")) then
		if player:GetSubfaith() == "Voltism" and cwBeliefs and (player:HasBelief("the_storm") or player:HasBelief("the_paradox_riddle_equation")) then
			...
			if (faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Order of Eternal Penance") then
				if (gender == "his") then
			...
			elseif (faction == "Darklanders" and player:GetSubfaction() != "Immortal") then
				if (gender == "his") then
					player:EmitSound("prelude/darklandervoice/ellis_combat_stun_0"..math.random(1, 5)..".ogg", 90, pitch)
				else
					player:EmitSound("prelude/darklandervoice/nadide_stun_0"..math.random(1, 6)..".ogg", 90, pitch)
				end
			elseif ((faction == "Darklanders" or faction == "Children of Satan") and player:GetSubfaction() == "Immortal") then
				player:EmitSound("prelude/piggysqueals/pain/"..math.random(1, 5)..".ogg", 90, pitch)
			else
				if (gender == "his") then
...
function cwMelee:EntityTakeDamageAfter(entity, damageInfo)
	if damageInfo and (entity:IsPlayer() or entity.isTrainingDummy or entity.LVS) then
		...
		if !entity.LVS and damageInfo:IsDamageType(DMG_BURN) then
			entity:TakeFreeze(damageInfo:GetDamage() / 2)
			
			if entity:GetNWBool("bliz_frozen") and IsValid(entity.freezeEnt) and entity:IsPlayer() then
			...
			if attacker:IsPlayer() then
				...
				if IsValid(attackerWeapon) then
					...
					if weaponItemTable and weaponItemTable.attributes and table.HasValue(weaponItemTable.attributes, "grounded") then
						...
						if entity.LVS then
							damageInfo:ScaleDamage(1+((entity:GetVelocity():Length() / 500)));
						end
					...
					elseif weaponItemTable and weaponItemTable.attributes and table.HasValue(weaponItemTable.attributes, "subjugator") then
						if entity:IsPlayer() and (!attacker:GetNetVar("Parried") and !attacker:GetNetVar("Deflected")) then
							if entity:HasDisease("shackled") or (entity:GetCharacterData("collared") == true) then
								entity:TakeStability((entity:GetMaxStability() / 2))
							end
						end
					end
				end
	...
	if player:Alive() and !Clockwork.player:HasFlags(player, "M") and player:WaterLevel() < 3 then
		...
		if (!player.nextPainSound or player.nextPainSound < CurTime()) then
			...
			elseif(subfaction == "Immortal") then
				player:EmitSound("prelude/piggysqueals/pain/"..math.random(1,5)..".ogg", 90, pitch);
				player.nextPainSound = CurTime()+0.5;
			elseif faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Order of Eternal Penance"  then
				if gender == "Male" then
			...
			elseif faction == "Darklanders" then
				if gender == "Male" then
					player:EmitSound("prelude/darklandervoice/ellis_combat_pain_0"..math.random(1, 8)..".ogg", 90, pitch)
					player.nextPainSound = CurTime()+0.5
				else
					player:EmitSound("prelude/darklandervoice/SW_Southern_Hurt_0"..math.random(1, 13)..".ogg", 90, pitch)
					player.nextPainSound = CurTime()+0.5
				end
			elseif faction == "Hillkeeper" then
				if gender == "Male" then
...
function GM:PlayerPlayDeathSound(player, gender)
	...
	if !Clockwork.player:HasFlags(player, "M") then
		...
		if player:GetCharacterData("isThrall") then
			...
			player.nextPainSound = CurTime()+0.5;
		elseif subfaction == "Immortal" then
			player:EmitSound("prelude/piggysqueals/death/"..math.random(1,6)..".ogg", 90, pitch);
		elseif faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Order of Eternal Penance" then
			if gender == "Male" then
		...
		elseif faction == "Darklanders" then
			if gender == "Male" then
				player:EmitSound("prelude/darklandervoice/ellis_combat_death_0"..math.random(1, 9)..".ogg", 90, pitch)
			else
				player:EmitSound("prelude/darklandervoice/SW_Southern_Death_0"..math.random(1, 16)..".ogg", 90, pitch)
			end
		else
			if gender == "Male" then
