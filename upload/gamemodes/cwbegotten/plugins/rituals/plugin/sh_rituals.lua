...
local RITUAL = cwRituals.rituals:New("purifying_stone_rite");
	...
	RITUAL.onerequiredbelief = {"man_become_beast", "one_with_the_druids", "daring_trout", "shedskin", "flagellant", "acolyte", "soothsayer", "heretic", "libertines_ritual2", "sows_ritual2", "vultures_ritual2"}; -- Tier II Shared Ritual
...
RITUAL = cwRituals.rituals:New("tortured_spirit_rite");
	...
	RITUAL.onerequiredbelief = {"soothsayer", "heretic", "shedskin", "libertines_ritual2", "sows_ritual2", "vultures_ritual2"}; -- Tier II Faith of the Dark Ritual
...
RITUAL = cwRituals.rituals:New("fire_sigil_stone_rite");
	...
	RITUAL.onerequiredbelief = {"sorcerer", "watchful_raven", "emissary", "extinctionist", "libertines_ritual3", "sows_ritual3", "vultures_ritual3"}; -- Tier III Shared Ritual
...
RITUAL = cwRituals.rituals:New("summon_eddie");
	...
	function RITUAL:EndRitual(player)
		...
		if (trace.HitPos:Distance(player:GetShootPos()) <= 192) then
			...
			Schema:Summon(trace.HitPos, trace.HitNormal, function()
...
RITUAL = cwRituals.rituals:New("summon_ironclad");
	RITUAL.name = "(T3) Summon Demon (Ironclad)";
	RITUAL.description = "Summon a Begotten Thrall that has become the host of a hell demon. It will be hostile towards anyone not of the Faith of the Dark. 10 second cast time. Incurs 25 corruption and costs blood.";
	RITUAL.onerequiredbelief = {"sorcerer"}; -- Tier III Faith of the Dark Ritual
	RITUAL.requiredBeliefsSubfactionOverride = {["Rekh-khet-sa"] = {"embrace_the_darkness"}}; -- Tier III Faith of the Dark Ritual
	
	RITUAL.requirements = {"hellforged_steel_ingot", "tortured_spirit", "down_catalyst"};
	RITUAL.corruptionCost = 25;
	RITUAL.ritualTime = 10;
	RITUAL.experience = 35;
	
	function RITUAL:OnPerformed(player)
		Schema:EasyText(GetAdmins(), "tomato", player:Name().." has performed the 'Summon Demon' ritual, spawning an Ironclad near their position!");
	
		player:ModifyBloodLevel(-1337);
	end;
	function RITUAL:OnFail(player)
	end;
	function RITUAL:StartRitual(player)
	...
	function RITUAL:EndRitual(player)
		...
		if (trace.HitPos:Distance(player:GetShootPos()) <= 192) then
			...
			local entity = ents.Create("npc_bgt_ironclad");
			local playerFaith = player:GetFaith();
	...
	function RITUAL:EndRitual(player)
		...
		if (trace.HitPos:Distance(player:GetShootPos()) <= 192) then
			...
			for i, v in pairs(positions) do
				Schema:Summon(v, Angle(0,0,1), function()
...
RITUAL = cwRituals.rituals:New("mastication");
	RITUAL.name = "(T1) Cadeverous Mastication";
	RITUAL.description = "To truly know a person is to have their heart. Performing this ritual causes eating hearts to grant you health, stamina and partially restore your blood level. This lasts 60 minutes. Incurs 10 corruption.";
	RITUAL.onerequiredbelief = {"libertines_ritual1", "sows_ritual1", "vultures_ritual1"}; -- Tier I Faith of the Many Ritual
	
	RITUAL.requirements = {"down_catalyst", "down_catalyst", "belphegor_catalyst"};
	RITUAL.corruptionCost = 10;
	RITUAL.ritualTime = 10;
	RITUAL.experience = 30;
	
	function RITUAL:OnPerformed(player)
		player.masticationActive = true;

		timer.Create("MasticationTimer_"..player:EntIndex(), 3000, 1, function()
			if IsValid(player) then
				if player.masticationActive then
					player.masticationActive = nil;

					Clockwork.hint:Send(player, "The 'Cadeverous Mastication' ritual has worn off...", 10, Color(175, 100, 100), true, true);
				end
			end
		end);
	end;
...
RITUAL = cwRituals.rituals:New("sharedlifeleech");
	RITUAL.name = "(Unique) In Death, Is Life (Target)";
	RITUAL.description = "There is plenty to go around! Performing this ritual with another character of your subfaith will make you share health gain from lifeleech. This lasts 20 minutes. Incurs 15 corruption on yourself and the target.";
	RITUAL.onerequiredbelief = {"vultures_ritual3"}; -- Unique Vultures Ritual
	
	RITUAL.requirements = {"trinity_catalyst", "ice_catalyst", "trinity_catalyst"};
	RITUAL.corruptionCost = 15;
	RITUAL.ritualTime = 10;
	RITUAL.experience = 75;
	...
	function RITUAL:StartRitual(player)
		...
		if IsValid(target) and target:IsPlayer() then
			if target:Alive() then
				if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
					if (!player.swarmPartner) then
						if (!target.swarmPartner) then
							if(target:HasBelief("the_vultures")) then
								return true;
							else
								Schema:EasyText(player, "firebrick", "This character is not of the Vultures subfaith!");
							end
						else
							Schema:EasyText(player, "firebrick", "The target character already has the 'In Death, Is Life' ritual active!");
						end
					else
						Schema:EasyText(player, "firebrick", "Your character already has the 'In Death, Is Life' ritual active!");
					end
				else
					Schema:EasyText(player, "firebrick", "This character is too far away!");
				end
			...
			if target:Alive() then
				if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
					if (!player.swarmPartner) then
						if (!target.swarmPartner) then
							if (target:HasBelief("the_vultures")) then
								player.swarmPartner = target;
								target.swarmPartner = player;

								target:HandleNeed("corruption", 15);

								timer.Create("SwarmTimer_"..player:EntIndex(), 1200, 1, function()
									if IsValid(player) then
										if player.swarmPartner then
											player.swarmPartner = nil;
						
											Clockwork.hint:Send(player, "The 'In Death, Is Life' ritual has worn off...", 10, Color(175, 100, 100), true, true);
										end
									end
								end);

								timer.Create("SwarmTimer_"..target:EntIndex(), 1200, 1, function()
									if IsValid(target) then
										if target.swarmPartner then
											target.swarmPartner = nil;
						
											Clockwork.hint:Send(target, "The 'In Death, Is Life' ritual has worn off...", 10, Color(175, 100, 100), true, true);
										end
									end
								end);
...
RITUAL = cwRituals.rituals:New("coinsucker");
	RITUAL.name = "(Unique) Coinsucker";
	RITUAL.description = "Are you going coinsucking? Performing this ritual causes your melee attacks to steal coin. It will also decrease your melee attack delay by 10%. This lasts 20 minutes. Incurs 15 corruption.";
	RITUAL.onerequiredbelief = {"libertines_ritual3"}; -- Unique Libertines Ritual
	
	RITUAL.requirements = {"trinity_catalyst", "trinity_catalyst", "belphegor_catalyst"};
	RITUAL.corruptionCost = 15;
	RITUAL.ritualTime = 10;
	RITUAL.experience = 75;
	
	function RITUAL:OnPerformed(player)
		player.coinsuckerActive = true;

		timer.Create("CoinsuckerTimer_"..player:EntIndex(), 1200, 1, function()
			if IsValid(player) then
				if player.coinsuckerActive then
					player.coinsuckerActive = nil;

					Clockwork.hint:Send(player, "The 'Coinsucker' ritual has worn off...", 10, Color(175, 100, 100), true, true);
				end
			end
		end);
	end;
...
RITUAL:Register()
--[[
RITUAL = cwRituals.rituals:New("baptizedinfilth");
	RITUAL.name = "(Unique) Baptized in Filth";
	RITUAL.description = "Baptized in Filth. Performing this ritual causes your warcries to be replaced by projectile vomit, victims will be temporarily blinded and lose sanity. This lasts 20 minutes. Incurs 15 corruption.";
	RITUAL.onerequiredbelief = {"sows_ritual3"}; -- Unique Sows Ritual
	
	RITUAL.requirements = {"light_catalyst", "trinity_catalyst", "belphegor_catalyst"};
	RITUAL.corruptionCost = 15;
	RITUAL.ritualTime = 10;
	RITUAL.experience = 75;
	
	function RITUAL:OnPerformed(player)
		player.baptizedfilthActive = true;

		timer.Create("BaptizedFilthTimer_"..player:EntIndex(), 1200, 1, function()
			if IsValid(player) then
				if player.baptizedfilthActive then
					player.baptizedfilthActive = nil;

					Clockwork.hint:Send(player, "The 'Baptized in Filth' ritual has worn off...", 10, Color(175, 100, 100), true, true);
				end
			end
		end);
	end;
...
RITUAL = cwRituals.rituals:New("covenant_creation");
	RITUAL.name = "(T2) Covenant Creation";
	RITUAL.description = "Summon the ancient rites to form a covenant. Those bound by this pact will share a deeper connection, strengthening their collective power. Incurs 20 corruption.";
	RITUAL.onerequiredbelief = {"man_become_beast", "one_with_the_druids", "daring_trout", "shedskin", "flagellant", "acolyte", "soothsayer", "heretic", "libertines_ritual2", "sows_ritual2", "vultures_ritual2"}; -- Tier II Shared Ritual
	
	RITUAL.requirements = {"light_catalyst", "elysian_catalyst", "belphegor_catalyst"};
	RITUAL.corruptionCost = 20;
	RITUAL.ritualTime = 15;
	RITUAL.experience = 50;
	
	function RITUAL:OnPerformed(player)
		if cwWandererCovenants then
			cwWandererCovenants:SendCovenantCreatePrompt(player)
		end
	end;
	function RITUAL:OnFail(player)
	end;
...
RITUAL = cwRituals.rituals:New("covenant_invite");
	RITUAL.name = "(T2) Invite To Covenant";
	RITUAL.description = "Use the power of your covenant to extend an invitation to a nearby person you're looking at. If they accept, they will be bound to your group, sharing in its power and purpose. Incurs 5 corruption initially and then 20, 25, 40 corruption when your target accepts, variable on the differences between your faiths.";
	RITUAL.onerequiredbelief = {"repentant", "disciple", "soothsayer", "witch", "witch_druid", "honor_the_gods", "one_with_the_druids", "the_black_sea", "witch_druid", "libertines_ritual1", "sows_ritual1", "vultures_ritual1"}; -- Tier I Shared Ritual
	
	RITUAL.requirements = {"down_catalyst", "elysian_catalyst", "ice_catalyst"};
	RITUAL.corruptionCost = 5
	RITUAL.ritualTime = 10;
	RITUAL.experience = 25;
	...
	function RITUAL:OnPerformed(player)
		if cwWandererCovenants then
			cwWandererCovenants:CovenantInviteNearbyPlayer(player)
		end
	end;
	function RITUAL:OnFail(player)
	end;
