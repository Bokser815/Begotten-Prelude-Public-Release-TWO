...
function cwRituals:PlayerCharacterLoaded(player)
	...
	if player.masticationActive then
		...
		if timer.Exists("MasticationTimer_"..entIndex) then
			timer.Remove("MasticationTimer_"..entIndex);
		end
	end

	if player.swarmPartner then
		local target = player.swarmPartner

		if IsValid(target) then
			target.swarmPartner = nil;

			if timer.Exists("SwarmTimer_"..target:EntIndex()) then
				timer.Remove("SwarmTimer_"..target:EntIndex())
			end
		end

		player.swarmPartner = nil;
		
		if timer.Exists("SwarmTimer_"..entIndex) then
			timer.Remove("SwarmTimer_"..entIndex)
		end
	end
	...
	if player.coinsuckerActive then
		...
		if timer.Exists("CoinsuckerTimer_"..entIndex) then
			timer.Remove("CoinsuckerTimer_"..entIndex);
		end
	end
...
function cwRituals:FuckMyLife(entity, damageInfo)
	if !entity.opponent then
		...
		if IsValid(attacker) and attacker:IsPlayer() and attacker.coinsuckerActive then
			local entcash = entity:GetCash()
			local attcash = attacker:GetCash()

			if entcash > 0 then
				local coin = math.Round(math.max(1, entcash / 5)); -- this needs to be done better
				
                Clockwork.player:GiveCash(entity, -coin, "Coinsucked!")
				Clockwork.player:GiveCash(attacker, coin, "Coinsucking")
			end
		end
	end
end
