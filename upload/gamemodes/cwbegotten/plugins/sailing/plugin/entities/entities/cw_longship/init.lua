...
function ENT:Initialize()
	...
	self.soundloop = CreateSound(self,"fiend/wavesship.wav")
	self.soundloop:Play()
	self.soundloop:ChangePitch(80)
	self.soundloop:SetSoundLevel(1170)
end;
...
function ENT:Think()
	...
	if (!self.checkCooldown or self.checkCooldown < curTime) then
		...
		if (!self.nextpassivesound or self.nextpassivesound < CurTime()) and (self.location != "rough") then
			local soundtoplay = "fiend/shipcreakpassive"..math.random(1, 5)..".wav"
			self.nextpassivesound = (math.random(60, 90))+CurTime()
			self:EmitSound(soundtoplay,2000)
		end
