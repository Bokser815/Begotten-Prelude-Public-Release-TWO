						...
						if longshipEnt.destination == "hell" then
							...
							for k, v in player.Iterator() do
								if v:GetFaction() == "Children of Satan" then
									...
									Schema:EasyText(v, "red", "Your mind is abruptly overcome with feelings of dread. The Dark Lord's domain is about to be invaded by an unknown force.");
									v:HandleSanity(-15);
								end
							end
						end
...
function cwSailing:GorewatchBellRing(sailColor, crewSize)
	for k, player in ipairs(_player.GetAll()) do
		-- trying to limit the number of players that get double for looped in this inefficient ass function
		if player:GetCharacterData("LastZone") == "wasteland" or player:GetCharacterData("LastZone") == "gorewatch" then
			for k2, ent in pairs(ents.FindInSphere(player:GetPos(), config.Get("talk_radius"):Get() * 2)) do
				if ent:GetClass() == "cw_gorewatch_bell" then
					Clockwork.chatBox:Add(player, nil, "it", "A longship has been spotted with a "..sailColor.." sail! It has a "..crewSize.." crew size.");
					player:SendLua([[Clockwork.Client:EmitSound("ambient/alarms/warningbell1.wav", 80, 100)]]);
					break;
				end
			end
		end
