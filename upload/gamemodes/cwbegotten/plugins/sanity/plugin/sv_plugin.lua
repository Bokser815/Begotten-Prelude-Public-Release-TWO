...
function playerMeta:HandleSanity(amount)
	...
	local intoxication = self:GetNeed("intoxication")
	...
	if amount < 0 then
		...
		if self:HasBelief("vultures_belief2") then
			amount = amount * 1.15;
		end

		if self:HasBelief("vultures_belief3") then
			amount = amount * 1.25;
		end
		...
		if self:HasTrait("alcoholic") and intoxication <= 10 then
			amount = amount * 2.5;
		end

		if intoxication >= 10 then
			amount = amount * Lerp(intoxication / 100, 0.8, 1)
		end
	end
