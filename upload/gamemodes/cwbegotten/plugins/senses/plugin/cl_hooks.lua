...
function cwSenses:Think()
	...
	if (!Clockwork.Client.cwNextDeafCheck or curTime > Clockwork.Client.cwNextDeafCheck) then
		Clockwork.Client.cwNextDeafCheck = curTime + 0.05;
		if (Clockwork.Client:HasTrait("deaf")) then
			if (!self.deafnessSound) then
				self.deafnessSound = CreateSound(Clockwork.Client, "begotten/ui/morphine_dream_loop.wav");

				if (self.deafnessSound and !self.deafnessSound:IsPlaying()) then
					self.deafnessSound:Play();
				end;
			end;
		else
			if (self.deafnessSound) then
				self.deafnessSound:FadeOut(1);
				
				timer.Simple(1, function()
					self.deafnessSound = nil;
				end);
			end;
		end;
	end;
