...
function ENT:HUDPaintTargetID(x, y, alpha)
	y = Clockwork.kernel:DrawInfo("A beacon. This one is used to coordinate the movement of vehicles.", x, y, Clockwork.option:GetColor("white"), alpha);
end;
...
function ENT:Think()
	...
	if !self.nextSoundCheck or self.nextSoundCheck < curTime then
		if Clockwork.Client:GetPos():DistToSqr(self:GetPos()) < (600 * 600) then
			if !self.Sound then
				self.Sound = CreateSound(self, "fiend/vehiclespawner.wav");
