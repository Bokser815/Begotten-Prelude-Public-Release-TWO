...
function cwZombies:GetAdminESPInfo(info)
	if (Clockwork.ConVars.ZOMBIEESP:GetInt() == 1) then
		...
		for k, v in ipairs(ents.FindByClass("npc_bgt_player")) do
			local informationColor = Clockwork.option:GetColor("information");
			local maxHealth = v:GetMaxHealth();
			local health = v:Health();
			local aggressiveness = v:GetNWInt("Aggressiveness");
			...
			info[#info + 1] = {
				...
				text = {
					{text = v.PrintName, color = informationColor},
					{text = "Health: ["..health.." / "..maxHealth.."]", color = informationColor},
					{text = "Aggressiveness: ["..aggressiveness.." / 50]", color = informationColor},
				}
			};
		...
		for k, v in ipairs(ents.FindByClass("npc_bgt_drop_chopper_evil")) do
			local informationColor = Clockwork.option:GetColor("information");
