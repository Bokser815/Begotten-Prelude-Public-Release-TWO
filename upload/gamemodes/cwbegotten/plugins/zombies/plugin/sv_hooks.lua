				...
				if npc.XPValue then
					for i = 1, #attackers do
						...
						if IsValid(iAttacker) then
							...
							if iAttacker:GetFaith() == "Old Ways" then
								if npc.fistdamage then
									local level = iAttacker:GetCharacterData("alternatelevels", 0)+math.Round(npc.fistdamage/4)
									iAttacker:SetCharacterData("alternatelevels", level)
									iAttacker:SetSharedVar("alternatelevels",level)
								end
							end
