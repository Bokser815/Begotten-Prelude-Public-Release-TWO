...
function Schema:Initialize()
	...
	if render.GetHDREnabled() == false and (game.GetMap() == "rp_begotten3" or game.GetMap() == "bg_prelude") then
...
Schema.ansibleMessages = {"Man... am I hungry...", "They say babies taste the best.", "I can't see! I can't fucking see!", "Hail Satan! HAIL SATAN!!! HAIL SATAN!!!!", "I gotta scratch my ass, but the High Gatekeeper is watching...", "Shall we gather for anal slop and Papa Pete's tonight?", "This ain't so bad."}


function Schema:Think()
	if Clockwork.Client:GetNWBool("ansibleIn") == true then
		local curTime =  CurTime();
		if !self.nextAnsibleMessage or self.nextAnsibleMessage < curTime then
			self.nextAnsibleMessage = curTime + math.random(50, 300);
			
			Clockwork.chatBox:Add(nil, nil, Color(255,150,55), "*** "..self.ansibleMessages[math.random(1, #self.ansibleMessages)]);
		end
	end
	...
	if Clockwork.Client:Alive() then
		...
		elseif (entity:GetClass() == "prop_ragdoll") then
			...
			if player then
				...
				elseif clientSubfaction == "Slaver" and playerFaction ~= "Piston's Scrappers" and player:GetNetVar("tied") != 0 then
					for k,v in pairs(ents.FindInSphere(Clockwork.Client:GetPos(), 512)) do
						if v:GetClass() == "cw_salesman" and v:GetNetworkedString("Name") == "Slavemaster" then
							options["Sell into Slavery"] = "cw_scrapperSellSlave";
...
function Schema:GetProgressBarInfoAction(action, percentage)
	...
	elseif (action == "building") then
		return {text = "You are building something. Click to cancel.", percentage = percentage, flash = percentage > 75};
	...
	elseif (action == "readybolt") then
		return {text = "You are preparing a ballista bolt.", percentage = percentage, flash = percentage > 75};
	elseif (action == "readypacking") then
		return {text = "You are ramming gunpowder down the barrel.", percentage = percentage, flash = percentage > 75};
	elseif (action == "readyball") then
		return {text = "You are slotting in a cannonball.", percentage = percentage, flash = percentage > 75};
	elseif (action == "readycan") then
		return {text = "You are slotting in a canister shot shell.", percentage = percentage, flash = percentage > 75};
	elseif (action == "cocking") then
		return {text = "COCKING HAMMER...", percentage = percentage, flash = percentage < 10};
	elseif (action == "bloodTypeTesting") then
		return {text = "You are testing someone's blood type...", percentage = percentage, flash = percentage < 10};
	...
	elseif (action == "trephine") then
		return {text = "You are trepanning someone's skull. Click to cancel.", percentage = percentage, flash = percentage < 10};
	...
	elseif (action == "shackling_slave") then
		return {text = "You are shackling a slave. Click to cancel.", percentage = percentage, flash = percentage < 10};
	elseif (action == "unshackling_slave") then
		return {text = "You are unlocking shackles. Click to cancel.", percentage = percentage, flash = percentage < 10};
	elseif (action == "repairdeployable") then
		return {text = "You are repairing something. Click to cancel.", percentage = percentage, flash = percentage > 75};
	elseif (action == "entervehicle") then
		return {text = "You are entering a vehicle. Click to cancel.", percentage = percentage, flash = percentage > 75};
	elseif (action == "hell_teleporting") then
		return {text = "You are using dark magic to teleport to Hell. Click to cancel.", percentage = percentage, flash = percentage < 10};
	elseif (action == "filling_bucket") then
		return {text = "You are filling a bucket. Click to cancel.", percentage = percentage, flash = percentage < 10};
...
function Schema:DrawTargetPlayerStatus(target, alpha, x, y)
	...
	if (target:Alive()) then
		...
		elseif target:GetSharedVar("wounded") and Clockwork.Client:HasBelief("doctor") then
			mainStatus = gender.." is wounded.";
		end;
		...
		if (target:GetNetVar("collared") != 0) then
			if (Clockwork.player:GetAction(Clockwork.Client) == "uncollar") then
				mainStatus = gender.. " is having their bomb collar taken off.";
			else
				local untieText;
				
				if (target:GetShootPos():Distance(Clockwork.Client:GetShootPos()) <= 192) then
					if (Clockwork.Client:GetNetVar("collared") == 0) then
						mainStatus = "Press :+use: to uncollar "..thirdPerson..".";
				...
				if (!untieText) then
					mainStatus = gender.." is wearing a bomb collar.";
				end;
			end;
		elseif (Clockwork.player:GetAction(Clockwork.Client) == "collar") then
			mainStatus = gender.." is being bomb collared.";
		end;
...
function Schema:DrawTargetPlayerSubfaction(target, alpha, x, y)
	...
	if targetSubfaction and targetSubfaction ~= "" and targetSubfaction ~= "N/A" then
		...
		elseif targetFaction ~= "Children of Satan" and targetFaction ~= "Goreic Warrior" then
			...
			elseif targetSubfaction == "Shepherds of Light" then
				if playerSubfaction == targetSubfaction then
					subfactionText = "A fellow preacher of the Shepherds of Light.";
					textColor = Color(0, 255, 0, 255);
				else
					subfactionText = "A preacher of the Shepherds of Light.";
				end
			elseif playerFaction == "Gatekeeper" or playerFaction == "Holy Hierarchy" or playerFaction == "Hillkeeper" or playerFaction == "Order of Eternal Penance" then
			...
			elseif playerFaction == "Piston's Scrappers" then
				local gang = target:GetSharedVar("scrapperGang")
				if targetSubfaction == "Slaver" then
					if gang then
						if gang == Clockwork.Client:GetSharedVar("scrapperGang") then
							subfactionText = "A Slaver and fellow member of "..gang..".";
							textColor = Color(0, 255, 0, 255);
						else
							subfactionText = "A Slaver of "..gang..".";
						end
					else
						subfactionText = "A Slaver of Piston's Warband.";
					end
				elseif targetSubfaction == "Greaser" then
					if gang then
						if gang == Clockwork.Client:GetSharedVar("scrapperGang") then
							subfactionText = "A Greaser and fellow member of "..gang..".";
							textColor = Color(0, 255, 0, 255);
						else
							subfactiontext = "A Greaser of "..gang..".";
						end
					else
						subfactiontext = "A Greaser of Piston's Warband.";
					end
				end
...
function Schema:GetScreenTextInfo()
	...
	elseif (Clockwork.Client:GetNetVar("beingCollared")) then
		return {
			alpha = 255 - blackFadeAlpha,
			title = "A BOMB COLLAR IS BEING APPLIED TO YOU"
		};
...
function Schema:ModifyStatusEffects(tab)
	...
	if Clockwork.Client:HasTrait("deaf") then
		table.insert(tab, {text = "(-) Deaf", color = Color(200, 40, 40)});
	end
end
...
function Schema:PlayerButtonDown(player, button)
    if button == KEY_F7 and IsFirstTimePredicted() then
        if CLIENT then 
            if not IsValid(Clockwork.Client.cwLangMenu) then
                RunConsoleCommand("cw_open_language_menu")
            end
        end
    end
end;
			...
			if weaponStats["attack"] and weaponStats["defense"] then
				...
				if itemTable.attributes then
					...
					if table.HasValue(itemTable.attributes, "grounded") then
						frame:AddText("Grounded: Cannot attack while sprinting, but hitting enemies in effective range will discombobulate them, stopping them from sprinting for the next 3 seconds. Increases damage to moving vehicles dramatically.", Color(110, 30, 30), nil, 0.9);
					end
...
netstream.Hook("DarkWarhorn", function(data)
	if cwMusic then
		cwMusic:FadeOutAmbientMusic(2, 1);
	end
	
	Clockwork.Client:EmitSound("prelude/darklandervoice/tenosian_horn_wet_01.ogg", 60, 75);
