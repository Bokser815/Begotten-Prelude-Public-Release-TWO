...
-- for the bomb collars: resembles setfreq code so i placed it here ~e

local COMMAND = Clockwork.command:New();
COMMAND.tip = "Set the frequency at which you transmit sound to bomb collars. You must be looking at a player.";
COMMAND.flags = CMD_DEFAULT;
COMMAND.arguments = 1;
...
function COMMAND:OnRun(player, arguments)
	...
	if ( string.find(frequency, "^%d+$") ) then
		local num = tonumber(frequency)
		if (num >= 0 and num <= 100) then
			player:SetCharacterData("soundSignalFrq", frequency)
		else
			Clockwork.player:Notify(player, "The frequency must be between 0 and 100!");
		end;
	else
		Clockwork.player:Notify(player, "The transmitted frequency must be a whole number between 0 and 100!");
	end;
end;

Clockwork.command:Register(COMMAND, "SetSignalFrq");
