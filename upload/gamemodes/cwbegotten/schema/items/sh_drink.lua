...
local ITEM = Clockwork.item:New();
	...
	-- Called when a player uses the item.
	function ITEM:OnUse(player, itemEntity)
		if (player:HasTrait("marked") or math.random(1,10) == 1) and !player:HasBelief("favored") then
			local gender = player:GetGender();
			local ragdollEntity = player:GetRagdollEntity();

			Clockwork.chatBox:AddInTargetRadius(player, "me", "opens a bottle of Papa Pete's® Grape-Flavored Ice Cold Pop™ but suddenly, an explosion of grapeshot flavored pellets shoot out from the bottle, turning their head into a fine mist! Holy shit!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			...
			Clockwork.kernel:CreateBloodEffects(player:EyePos(), 1, player);
			...
			effect:SetOrigin(player:EyePos());
			effect:SetNormal(player:GetAimVector());
			util.Effect( "effect_awoi_smoke_pistol", effect );

			player:EmitSound("musket/musket4.wav", 100, 150)

			player:Kill()
			player:DeathCauseOverride("Got a special surprise from a Papa Pete's® Ice Cold Pop™!");
		else
...
local ITEM = Clockwork.item:New();
	...
	ITEM.needs = {hunger = 85, thirst = 100, intoxication = -40};
...
local ITEM = Clockwork.item:New();
	...
	ITEM.needs = {thirst = 100, intoxication = -10};
...
local ITEM = Clockwork.item:New();
	...
	ITEM.customFunctions = {"Water Planter"};
