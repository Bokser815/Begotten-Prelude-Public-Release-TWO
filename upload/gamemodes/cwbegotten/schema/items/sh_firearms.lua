...
local ITEM = Clockwork.item:New("firearm_base");
	...
	ITEM.components = {breakdownType = "meltdown", items = {"scrap", "scrap", "scrap"}, scrapperItems = {"scrap", "scrap"}};
...
local ITEM = Clockwork.item:New("firearm_base");
	...
	ITEM.components = {breakdownType = "meltdown", items = {"steel_chunks", "scrap", "scrap", "scrap"}, scrapperItems = {"steel_chunks", "scrap", "scrap"}};
...
local ITEM = Clockwork.item:New("firearm_base");
    ITEM.name = "Scavenger Revolver";
    ITEM.model = "models/weapons/darky_m/rust/w_revolver.mdl";
    ITEM.weight = 2;
    ITEM.uniqueID = "begotten_scavenger_revolver";
    ITEM.description = "This gun is the biggest western trigger you've ever experienced. A hand-tooled handgun made out of crude scrap and iron. It appears that it needs to be cocked in order to fire.";
    ITEM.iconoverride = "materials/begottenprelude/ui/itemicons/w_revolver.png"
    ITEM.isAttachment = true;
	ITEM.attachmentBone = "ValveBiped.Bip01_R_Thigh";
	ITEM.attachmentOffsetAngles = Angle(68.62, 0, 357.02);
	ITEM.attachmentOffsetVector = Vector(-6.4, -4.07, 5.3);

    ITEM.ammoCapacity = 6;
    ITEM.ammoTypes = {"Pop-a-Shot", "Scrapshot"};
    ITEM.firearmType = "Handgun";
    ITEM.isRevolver = true;
    ITEM.reloadTime = 5; -- Seconds (for one round)
	ITEM.reloadSounds = {"scraprevolver/load_bullet.ogg", "weapons_trashcan/rel.wav", "weapons/357/357_spin1.wav"}
	ITEM.components = {breakdownType = "meltdown", items = {"scrap", "scrap", "steel_chunks", "steel_chunks"}};

	ITEM.itemSpawnerInfo = {category = "Firearms", rarity = 1337, supercrateItems = {["pop-a-shot"] = {min = 3, max = 12}, ["scrapshot"] = {min = 6, max = 24}}};
ITEM:Register()

local ITEM = Clockwork.item:New("firearm_base");
	ITEM.name = "Scrap Blowtorch";
	ITEM.model = "models/props_silo/welding_torch2.mdl";
	ITEM.weight = 3.5;
	ITEM.uniqueID = "begotten_lvsfueler";
	ITEM.description = "A very expensive mechanism, made out of scrap metal. It is a miracle of scrap smithing, but more importantly it is capable of making field repairs.";
	ITEM.iconoverride = "materials/begotten/ui/itemicons/welding_torch2.png"
	ITEM.isAttachment = false;
	ITEM.attachmentBone = "ValveBiped.Bip01_Spine2";
	ITEM.attachmentOffsetAngles = Angle(0, 351.47, 90);
	ITEM.attachmentOffsetVector = Vector(-2.12, 3.54, -0.71);

	ITEM:AddData("smeltamount", 0, true);
	
	ITEM.ammoCapacity = 1;
	ITEM.ammoTypes = {"SmeltStick"};
	ITEM.firearmType = "Handgun";
	ITEM.isRevolver = true;
	ITEM.reloadTime = 1; -- Seconds (for one round)
