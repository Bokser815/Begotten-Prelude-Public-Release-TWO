...
local ITEM = Clockwork.item:New("medical_base");
	...
	ITEM.useText = "Drill";
	ITEM.useSound = "bandaging_1.wav";
	ITEM.description = "A rusty trephine, used for drilling holes in skulls to relieve pressure.";
	ITEM.iconoverride = "materials/begotten/ui/itemicons/ampoule.png"
	ITEM.uniqueID = "trephine"
	...
	ITEM.itemSpawnerInfo = {category = "Medical", rarity = 600, bNoSupercrate = true};
	ITEM.limbs = {HITGROUP_HEAD};

	function ITEM:OnUse(player, itemEntity)
		local trace = player:GetEyeTraceNoCursor();
		local target = Clockwork.entity:GetPlayer(trace.Entity);

		if target and target:GetRagdollState() == RAGDOLL_KNOCKEDOUT then
			Clockwork.chatBox:AddInTargetRadius(player, "me", "begins to trepan the skull of "..target:Name()..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

			Clockwork.player:SetAction(player, "trephine", 15, 5, function()

				if player:HasBelief("surgeon") then
					if target:HasDisease("brain_bleeding") then
						target:TakeDisease("brain_bleeding");

						Clockwork.chatBox:AddInTargetRadius(target, "me", "leaks blood from their skull, as if some had been building up previously.", target:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					else
						Clockwork.chatBox:AddInTargetRadius(player, "me", "drills a hole into "..target:Name().."'s skull.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					end
				else
					Clockwork.chatBox:AddIntargetRadius(player, "me", "slips and stabs "..target:Name().." in the brain with a trephine!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					target:ScriptedDeath("Died due to botched trepanation.");
				end
			end);
		else
			Schema:EasyText(player, "firebrick", "You can only use this on unconscious individuals!");
		end
	end;
ITEM:Register();
...
local ITEM = Clockwork.item:New();
	ITEM.name = "Medicinal Blood Test Kit";
	ITEM.category = "Tools";
	ITEM.cost = 250;
	ITEM.model = "models/kali/miscstuff/stalker/aid/first aid kit.mdl";
	ITEM.weight = 0.4;
	ITEM.useText = "Test Blood";
	ITEM.description = "An ancient device used to test another person's blood type.";
	ITEM.stackable = false;
	ITEM.customFunctions = {"Test Blood (Self)"};
	...
	function ITEM:OnUse(player, itemEntity)
		...
		if entity:IsPlayer() then
			Clockwork.chatBox:AddInTargetRadius(player, "me", "begins testing the blood type of the person before them.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			Clockwork.player:SetAction(player, "bloodTypeTesting", 5, 5, function()
				local bloodType = entity:GetCharacterData("BloodType");

				--debug
				print(bloodType);

				bloodType = cwMedicalSystem.bloodTypeToString[bloodType];

				Schema:EasyText(player, "indianred", "The taken sample reveals the composition of the blood: "..bloodType..".");
			end);
		else
			Schema:EasyText(player, "firebrick", "You are not looking at a valid character!");
		end
	...
	function ITEM:OnCustomFunction(player, name)
		if (name == "Test Blood (Self)") then
			Clockwork.chatBox:AddInTargetRadius(player, "me", "begins testing their own blood type.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			Clockwork.player:SetAction(player, "bloodTypeTesting", 5, nil, function()
				local bloodType = player:GetCharacterData("BloodType");
