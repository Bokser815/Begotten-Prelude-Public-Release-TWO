local ITEM = Clockwork.item:New("weapon_base");
	...
	ITEM.ammoType = "IronJavBolt";
	if ITEM.customFunctions then
		table.insert(ITEM.customFunctions, "Ready As Bolt");
	else
		ITEM.customFunctions = {"Ready As Bolt"};
	end
	
	if (SERVER) then
		function ITEM:OnCustomFunction(player, name)
			if (name == "Ready As Bolt") then
							...
							if ent.Owner != player then
								if ent:GetPos():Distance(player:GetPos())<100 then -- check distance
									if self:GetCondition()>20 then
										...
										Schema:EasyText(player, "chocolate", "This bolt is in no condition to be loaded!");
										return false
									end
								else
...
local ITEM = Clockwork.item:New("weapon_base");
	...
	ITEM.weight = 3;
	ITEM.uniqueID = "begotten_javelin_ballistabolt";
	ITEM.category = "Throwables";
	ITEM.description = "A large ballista bolt. Can be thrown as a javelin in a pinch.";
...
local ITEM = Clockwork.item:New("weapon_base");
	ITEM.name = "Scrap Javelin";
	ITEM.model = "models/lostcoast/fisherman/harpoon offset.mdl";
	ITEM.weight = 3;
	ITEM.uniqueID = "begotten_javelin_scrap";
	ITEM.category = "Throwables";
	ITEM.description = "A makeshift scrap javelin commonly used by scrappers. The crude design resembles a fisherman's hook.";
	--ITEM.iconoverride = "begotten/ui/itemicons/iron_javelin.png"
	ITEM.isAttachment = true;
	ITEM.isJavelin = true;
	ITEM.attachmentBone = "ValveBiped.Bip01_Spine2";
	...
	ITEM.components = {breakdownType = "meltdown", items = {"scrap"}}; -- use "breakdown" for other type
									...
									if self:GetCondition()>20 then
										Clockwork.player:SetAction(player, "readybolt", 2, nil, function()
											if IsValid(ent) and IsValid(player) then	
												...
												player:TakeItem(self, true);
												player:SetForcedAnimation("ThrowItem", 1.2)
												Clockwork.chatBox:AddInTargetRadius(player, "me", "places a scrap javelin next to the ballista.", player:GetPos(), config.Get("talk_radius"):Get() * 2); -- fix double name
											end
										end);
									else
										Schema:EasyText(player, "chocolate", "This javelin is in no condition to be loaded!");
										return false
									end
								else
									Schema:EasyText(player, "chocolate", "You need to be closer to reload!");
...
local ITEM = Clockwork.item:New("weapon_base");
	ITEM.name = "Assassin's Throwing Dagger";
	ITEM.model = "models/demonssouls/weapons/secret throwing dagger offset.mdl";
	ITEM.weight = 0.8;
	ITEM.uniqueID = "begotten_javelin_throwing_dagger_assassin";
	ITEM.category = "Throwables";
	ITEM.description = "An awesome fucking throwing dagger.";
	--ITEM.iconoverride = "begotten/ui/itemicons/throwing_dagger.png"
	ITEM.isJavelin = true;
	ITEM.canUseShields = true;
