...
// irrelevent for prelude currently - sprite
...
local COMMAND = Clockwork.command:New("GetEntityInfo")
	COMMAND.tip = "Gets the model, position and angles of the entity you're looking at. Intended to help setup entities in the schema."
	COMMAND.access = "o"

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if IsValid(player:GetEyeTraceNoCursor().Entity) then
			local model = player:GetEyeTraceNoCursor().Entity:GetModel()
			local position = player:GetEyeTraceNoCursor().Entity:GetPos() 
			local angles = player:GetEyeTraceNoCursor().Entity:GetAngles()
			
			local entityString = "{model = \"".. tostring(model) .."\", pos = Vector(".. tostring(position) .."), angles = Angle(".. tostring(angles) ..")},"
			Schema:EasyText(player, "khaki", entityString)
			netstream:Start(player, "cwClipboardText", entityString)
		else
			Schema:EasyText(player, "peru", "Entity not found!")
		end
	end
COMMAND:Register()
...
local COMMAND = Clockwork.command:New("BlowWarhorn");
	...
	COMMAND.access = "s";
	COMMAND.arguments = 1;
	COMMAND.text = "<gore/dark>"

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if arguments[1] == "gore" or arguments[1] == "dark" then
			...
			if lastZone == "wasteland" or lastZone == "tower" or lastZone == "theater" then
				for k, v in _player.Iterator() do
					if IsValid(v) and v:HasInitialized() then
						...
						if vLastZone == "wasteland" or vLastZone == "tower" or vLastZone == "theater" then
							...
							elseif arguments[1] == "dark" then
								Clockwork.chatBox:Add(v, nil, "event", "The ground rumbles as the harrowing sound of a Darklander horn pierces the sky.");
								netstream:Start(v, "DarkWarhorn");
							end
						end
					end
				end
			...
			Schema:EasyText(player, "peru", "Invalid argument, use 'gore' or 'dark'!");
		end
	end;
COMMAND:Register();
			...
			if (entity:GetClass() == "cw_coinslot") then
				if arguments[1] and tonumber(arguments[1]) then
					...
					if (cash and Clockwork.player:CanAfford(player, cash)) then
						...
						if cwBeliefs then
							if player:GetFaction() == "Gatekeeper" or player:GetFaction() == "Order of Eternal Penance" then
...
local COMMAND = Clockwork.command:New("MakeGang")
	COMMAND.tip = "Create a Scrapper Gang."
	COMMAND.text = "<Gang Name, no quotations>";
	COMMAND.access = "s"
	COMMAND.arguments = 1;

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if player:GetFaction() == "Piston's Scrappers" and string.find(player:Name(), "Warlord") and not player:GetCharacterData("scrapperGang") then
			local gangName = tostring(arguments[1]);

			if Schema and Schema.scrapperGangs then
				if Schema.scrapperGangs[gangName] then
					Schema:EasyText(player, "maroon", "This gang name already exists!");
				else
					Schema:AddGang(gangName, player:Name());
					player:SetCharacterData("scrapperGang", gangName);
					player:SetCharacterData("gangLeader", true);
					player:SetSharedVar("scrapperGang", gangName);
					Clockwork.player:NotifyAdmins("operator", player:Name().." has created the scrapper gang "..gangName..".");
				end
			else
				Clockwork.player:NotifyAdmins("operator", "The \"Schema.scrapperGangs\" system has not initialized properly; "..player:Name().. " has attempted to make one. Please investigate.")
			end
		else
			Schema:EasyText(player, "maroon", "You are not the correct faction and/or rank to do this!");
		end
	end
COMMAND:Register()

local COMMAND = Clockwork.command:New("GangEnlist")
	COMMAND.tip = "Ask a Scrapper to join your gang."

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if player:GetFaction() == "Piston's Scrappers" and string.find(player:Name(), "Warlord") and player:GetCharacterData("scrapperGang") then
			local target = Clockwork.entity:GetPlayer(player:GetEyeTraceNoCursor().Entity);
			local gangName = player:GetCharacterData("scrapperGang");

			if target and target:Alive() and (target:GetShootPos():Distance(player:GetShootPos()) <= 192) and target:GetFaction() == "Wanderer" then
				Clockwork.dermaRequest:RequestConfirmation(target, "Gang Enlistment", player:Name().." has invited you to join the "..gangName.." Scrapper Gang!", function()
					player:SetCharacterData("scrapperGang", gangName);
					player:SetSharedVar("scrapperGang", gangName);

					if Schema.scrapperGangs[gangName] then
						Schema:UpdateGangMemberCount(gangName, 1);
						local members = Schema:GetMembers(gangName);
						table.insert(members, target:Name());
						Schema:UpdateMembers(gangName);
					end
				end);
			end
		end
...
local COMMAND = Clockwork.command:New("LeaveGang")
	COMMAND.tip = "Leave the Scrapper Gang you are currently part of."

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if player:GetFaction() == "Piston's Scrappers" then
			if (player:GetSharedVar("scrapperGang") or player:GetCharacterData("scrapperGang")) then
				if not player:GetCharacterData("gangLeader") then
					local gang = player:GetCharacterData("scrapperGang");
					player:SetSharedVar("scrapperGang", nil);
					player:SetCharacterData("scrapperGang", nil);
					if Schema.scrapperGangs[gang] then
						if Schema.scrapperGangs[gang].memberCount == 1 then
							Schema:RemoveGang(gang);
						elseif Schema.scrapperGangs[gang].memberCount > 1 then
							Schema:UpdateGangMemberCount(gang, -1);
							local pos;
							local members = Schema:GetMembers(gang);
							...
							for k,v in members do
								if v == player:Name() then
									...
									if pos then
										...
										Schema:UpdateMembers(gang, members);
										break;
									end
								end
							end;
					...
					Schema:EasyText(player, "maroon", "You cannot leave the gang you have created!");
				end
			else
				Schema:EasyText(player, "maroon", "You are not part of a Scrapper Gang!");
			end
		else
			Schema:EasyText(player, "maroon", "You are not a Scrapper!");
		end
	end
COMMAND:Register()

local COMMAND = Clockwork.command:New("ListGangs")
	COMMAND.tip = "List all Scrapper Gangs. This may be extensive.";
	COMMAND.access = "s";

	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		if Schema and Schema.scrapperGangs then
			for k,v in pairs(Schema.scrapperGangs) do
				Schema:EasyText(player, "lightslateblue", k);
			end
		end
	end
COMMAND:Register()

local COMMAND = Clockwork.command:New("ListGangMembers")
	COMMAND.tip = "List all members of a Scrapper Gang.";
	COMMAND.access = "s";
	COMMAND.arguments = 1;
	COMMAND.text = "<string GangName>"
	...
	function COMMAND:OnRun(player, arguments)
		...
		if Schema.scrapperGangs[gangName] then
			local gangMembers = Schema:GetMembers(gangName);
			Schema:EasyText(player, "lightslateblue", "Gang Leader: "..Schema.scrapperGangs[gangName].leader);
			local members = "";
			if gangMembers then
				for k,v in pairs(gangMembers) do
					members = members..v..",";
				end
				Schema:EasyText(player, "lightslateblue", members)
			end
		end
	end
COMMAND:Register()

local COMMAND = Clockwork.command:New("DeleteGang")
	COMMAND.tip = "Delete a Scrapper Gang.";
	COMMAND.access = "s";
	COMMAND.arguments = 1;
	COMMAND.text = "<string GangName>";
	...
	function COMMAND:OnRun(player, arguments)
		...
		if Schema.scrapperGangs[gangName] then
			Schema:RemoveGang(gangName);
			Clockwork.player:NotifyAdmins("operator", player:Name().." has deleted the scrapper gang "..gangName..".");
		else
			Schema:EasyText(player, "maroon", "That is a not a valid gang name!");
		end
	end
COMMAND:Register()

local COMMAND = Clockwork.command:New("ForceLeaveGang")
	COMMAND.tip = "Force an individual to leave a scrapper gang.";
	COMMAND.access = "s";
	COMMAND.arguments = 1;
	...
	-- Called when the command has been run.
	function COMMAND:OnRun(player, arguments)
		local charName = tostring(arguments[1]);
		local target = Clockwork.player:FindByID(charName);

		if not target then
			Schema:EasyText(player, "maroon", "That is not a valid player!");
		else
			local gang = target:GetCharacterData("scrapperGang");

			if not gang then
				Schema:EasyText(player, "maroon", "This player is not part of a gang!");
			else
				if not Schema.scrapperGangs[gang] then
					Schema:EasyText(player, "maroon", "Something has gone wrong, and this player's gang could not be found in the Schema.scrapperGangs table. Contact Vlad.");
				else
					local members = Schema:GetMembers(gang);
					Schema:UpdateGangMemberCount(gang, -1);

					if Schema.scrapperGangs[gang].memberCount == 0 then
						Schema:DeleteGang(gang);
						return;
					end
			...
			if faction == "Goreic Warrior" then
				for _,v in pairs(_player.GetAll()) do
					...
					if (lastZone == "gore" or lastZone == "gore_tree" or lastZone == "gore_hallway") then
						...
						v:SendLua([[Clockwork.Client:EmitSound("event/summonhorn.mp3", 60, 100)]]);
						util.ScreenShake(v:GetPos(), 1, 5, 10, 1024);
					end
				end
			else
			...
			if faction == "Goreic Warrior" then
				for _,v in pairs(_player.GetAll()) do
					...
					if (lastZone == "gore" or lastZone == "gore_tree" or lastZone == "gore_hallway") then
						...
						util.ScreenShake(v:GetPos(), 1, 5, 11, 1024);
						v:SendLua([[Clockwork.Client:EmitSound("event/raidhorn.mp3", 60, 100)]]);
					end
				end
			else
				Schema:EasyText(player, "firebrick", "You are not the correct faction to blow the Goreic Gathering Horn!");
...
local COMMAND = Clockwork.command:New("SpawnHellPortalEntrance");
COMMAND.tip = "Spawn a Hell Entrance Portal as an admin with a custom despawn time.";
COMMAND.text = "[int despawnTime]";
COMMAND.access = "s";
COMMAND.optionalArguments = 1;
...
function COMMAND:OnRun(player, arguments)
	...
	if not arguments[1] then
		despawnTime = math.random(10, 20);
	else
		despawnTime = tonumber(arguments[1]);
	end

	local pos = (player:GetPos() + (player:GetForward() * 75));

	cwHellGrinder:SpawnEnterHellPortal(pos, despawnTime);
end
COMMAND:Register();
...
local COMMAND = Clockwork.command:New("HookahPuff");
	COMMAND.tip = "Take a puff from the Communal Hookah.";
	COMMAND.text = "[bool WickedPuff]";
	COMMAND.flags = CMD_DEFAULT;
	COMMAND.optionalArguments = 1;
	...
	function COMMAND:OnRun(player, arguments)
		local trace = player:GetEyeTrace();
		local coughingNoises = {"ambient/voices/cough1.wav", "ambient/voices/cough2.wav", "ambient/voices/cough3.wav", "ambient/voices/cough4.wav"};

		if (trace.Entity) then
			local entity = trace.Entity;
		
			if (entity:GetClass() == "cw_hookah") then
				local faction = player:GetFaction();
				local cash = player:GetCash();
				if !arguments[1] then
					if cash >= 100 then
						Schema:ModifyHookahTreasury(100);
						Clockwork.player:GiveCash(player, -100, nil, true);
						local effect = EffectData();
						local Forward = player:GetForward()
						local Right = player:GetRight()
						effect:SetOrigin(player:GetShootPos() + (Forward * 25) + (Right * 2));
						effect:SetNormal( player:GetAimVector());
						util.Effect( "effect_awoi_smoke_pistol", effect);
						player:HandleNeed("sleep", -10);

						if faction == "Darklanders" or faction == "Qarin Bloodsworn" then
							player:HandleXP(50);
							Clockwork.chatBox:AddInTargetRadius(player, "me", "inserts coin and takes a hit from the Communal Hookah. They inhale deeply, holding the breath for a few moments before calmly releasing a cloud of smoke from their mouth.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						else
							Clockwork.chatBox:AddInTargetRadius(player, "me", "inserts coin and takes a hit from the Communal Hookah. After taking a single breath in, they release a flurry of dry coughs.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
							local randomCough = (math.ceil(math.Rand(0, #coughingNoises)));
							player:EmitSound(coughingNoises[randomCough]);
						end
					else 
						Schema:EasyText(player, "peru", "You do not have enough coin to take a puff!");
					end
				else
					if cash >= 500 then
						Schema:ModifyHookahTreasury(500);
						Clockwork.player:GiveCash(player, -500, nil, true);
						local effect = EffectData();
						local Forward = player:GetForward()
						local Right = player:GetRight()
						effect:SetOrigin(player:GetShootPos() + (Forward * 25) + (Right * 2));
						...
						if faction == "Darklanders" or faction == "Qarin Bloodsworn" then
							player:HandleXP(300);
							Clockwork.chatBox:AddInTargetRadius(player, "me", "inserts coin and takes a wicked hit from the Communal Hookah. They inhale deeply for an extended period of time, holding the breath for a few moments before slowly releasing a large cloud of smoke from their mouth.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						else
							Clockwork.chatBox:AddInTargetRadius(player, "me", "inserts coin and takes a wicked hit from the Communal Hookah. After taking a single breath in, they release a flurry of violent, dry coughs.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
							local randomCough = (math.ceil(math.Rand(0, #coughingNoises)));
							player:EmitSound(coughingNoises[randomCough]);
						end
					else
						Schema:EasyText(player, "peru", "You do not have enough coin to take a wicked puff!");
					end
				end
			else
				Schema:EasyText(player, "peru", "You must be looking at the Communal Hookah to take a puff!");
			end;
		end;
	end;
COMMAND:Register();
...
local COMMAND = Clockwork.command:New("HookahCollect");
	COMMAND.tip = "Collect from the Communal Hookah.";
	COMMAND.text = "<number Coin>";
	COMMAND.flags = CMD_DEFAULT;
	COMMAND.arguments = 1;
			...
			if (trace.Entity) then
				...
				if (entity:GetClass() == "cw_hookah") then
					if arguments[1] and tonumber(arguments[1]) then
						...
						if Schema.hookahTreasury >= cash and cash > 0 then
							Clockwork.player:GiveCash(player, cash, nil, true);
							Schema:ModifyHookahTreasury(-cash);
							
							Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, player:Name().." has collected "..cash.." coin from the Communal Hookah. The Hookah treasury now sits at "..Schema.hookahTreasury..".");
