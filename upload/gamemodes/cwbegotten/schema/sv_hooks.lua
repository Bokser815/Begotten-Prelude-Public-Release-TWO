...
local bMap = map == "rp_begotten3" or map == "rp_district21" or map == "bg_prelude";
...
function Schema:PlayerCanInteractCharacter(player, action, character)
	...
	if action == "delete" then
		...
		if gang then
			if Schema.scrapperGangs[gang].members and Schema.scrapperGangs[gang].members > 1 then
			...
			elseif Schema.scrapperGangs[gang] and Schema.scrapperGangs[gang].members == 1 then
...
function Schema:ClockworkInitPostEntity()
	...
	self.scrapperGangs = Clockwork.kernel:RestoreSchemaData("scrapperGangs") or {};
	self.hookahTreasury = Clockwork.kernel:RestoreSchemaData("hookahTreasury")[1] or 0;
end;
...
function Schema:SaveData() 
	...
	if self.hookahTreasury then
		Clockwork.kernel:SaveSchemaData("hookahTreasury", {self.hookahTreasury});
	end
	...
	if self.scrapperGangs then
		Clockwork.kernel:SaveSchemaData("scrapperGangs", self.scrapperGangs);
	end
end;
...
function Schema:EntityHandleMenuOption(player, entity, option, arguments)
	...
	if entity:IsPlayer() and entity:GetNetVar("tied") != 0 then
		...
		elseif arguments == "cw_scrapperSellSlave" and playerSubfaction == "Slaver" and entFaction ~= "Piston's Scrappers" then
			for k, v in pairs(ents.FindInSphere(player:GetPos(), 512)) do
				if v:GetClass() == "cw_salesman" and v:GetNetworkedString("Name") == "Slavemaster" then
					if entity:GetCharacterData("slave") then
						local acceptedSlave;

						Clockwork.dermaRequest:RequestConfirmation(entity, "Slavery", "Do you wish to live on as a slave?", function()
							acceptedSlave = true;
						end);
						...
						if not acceptedSlave then
							-- TODO maybe multiple slavers get coin
							Clockwork.player:GiveCash(player, entity:GetCharacterData("level", 1) * 8, "Sold Slave");
							...
							player:GiveItem(Clockwork.item:CreateInstance("slave_shackles"));
							Schema:EasyText(player, "olivedrab", "Your slave has chosen to live on.")
							...
							entity:SetCharacterData("slave", true);


						end;
					else
						Schema:EasyText(player, "firebrick", "That slave is already part of our labor force!");
					end;
				end;
			end;
			...
			for k, v in pairs(ents.FindInSphere(player:GetPos(), 512)) do
				if v:GetClass() == "cw_bounty_board" then
					...
					if cwBeliefs then
						...
						local playerCash = player:GetCash();
						local entityCash = entity:GetCash();
						...
						if player:HasBelief("libertines_ritual3") then
							if (player:GetGender() == GENDER_MALE and entity:GetGender() == GENDER_FEMALE) or (player:GetGender() == GENDER_FEMALE and entity:GetGender() == GENDER_MALE) then
								killXP = killXP * 2;
							end
						end
						if player:HasBelief("libertines_ritual2") then
							if playerCash < entityCash then
								killXP = killXP * 2;
							end
						end
					...
					elseif playerFaction == "Order of Eternal Penance" then
						playername = "an unknown Extinctionist"
					elseif playerFaction == "Darklander" then
						playername = "an unknown Darklander"
...
function Schema:KeyPress(player, key)
	if (key == IN_USE) then
		...
		if (IsValid(target)) then
			...
			if (target and player:GetNetVar("tied") == 0) and (player:GetFaction() == "Piston's Scrappers") then
				if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
					if (target:GetNetVar("collared") != 0) then
					
						Clockwork.player:SetAction(player, "uncollar", untieTime);
						
						Clockwork.player:EntityConditionTimer(player, target, entity, untieTime, 192, function()
							return player:Alive() and !player:IsRagdolled() and !player:HasGodMode() and player:GetNetVar("tied") == 0;
						end, function(success)
							if (success) then
								self:CollarPlayer(target, false);
								
								player:GiveItem(item.CreateInstance("bomb_collar"));
							end;
							
							Clockwork.player:SetAction(player, "uncollar", false);
						end);
					end;
				end;
			end;
	...
	elseif (key == IN_ATTACK) then
		...
		if (action == "reloading") or (action == "mutilating") or (action == "skinning") or (action == "building") or (action == "entervehicle") then
...
function Schema:PlayerDoesRecognisePlayer(player, target, status, isAccurate, realValue)
	...
	if targetFaction == "Holy Hierarchy" then
		return true;
	elseif targetFaction == "Gatekeeper" or targetFaction == "Pope Adyssa's Gatekeepers" or targetFaction == "Hillkeeper" or targetFaction == "Order of Eternal Penance" then
		if playerFaction == "Gatekeeper" or playerFaction == "Pope Adyssa's Gatekeepers" or playerFaction == "Hillkeeper" or playerFaction == "Holy Hierarchy" or playerFaction == "Order of Eternal Penance" then
			return true;
		end
	elseif targetFaction == "Goreic Warrior" and playerFaction == "Goreic Warrior" then
		return true;
	elseif targetFaction == "Darklanders" and playerFaction == "Darklanders" then
		return true;
...
function Schema:PlayerCanUseDoor(player, door)
	...
	if doors then
		...
		if doors["tower"] and table.HasValue(doors["tower"], doorName) then
			...
			if faction ~= "Holy Hierarchy" and faction ~= "Gatekeeper" and faction ~= "Hillkeeper" and faction ~= "Pope Adyssa's Gatekeepers" and faction ~= "Order of Eternal Penance"  then
...
local langMap = {
    ["Goreic Warrior"]    = "runespeak",
    ["Children of Satan"] = "darkspeak",
    ["Holy Hierarchy"]           = "highglazic"
}
...
function Schema:ChatBoxAdjustInfo(info)
	if (IsValid(info.speaker) and info.speaker:HasInitialized()) then
		...
		if table.HasValue(langClasses, info.class) then
            if info.data and info.data.translated then return end

            local speaker = info.speaker
            local lang = speaker.speakingLanguage
            local originalText = tostring(info.text)  
            local translatedText = originalText  

            local words = string.Explode(" ", originalText)

            if lang == "runespeak" then
                local runelang = {} 
                for i, word in ipairs(words) do
                    local punctuationStart = word:match("^[%p%c]+") or ""
                    local punctuationEnd   = word:match("[%p%c]+$") or ""
                    local strippedWord = word:sub(#punctuationStart + 1, #word - #punctuationEnd)
                    local lowerWord    = strippedWord:lower()
                    local gruntWord = cwLanguage.gore_dict[lowerWord]
                    if not gruntWord then
                        local length = math.random(3, 6)
                        local grunt = {}
                        for j = 1, length do
                            table.insert(grunt, cwLanguage.gore_syllables[math.random(#cwLanguage.gore_syllables)])
                        end
                        gruntWord = table.concat(grunt)
                    end
                    if #strippedWord > 0 then
                        local firstIsUpper = (strippedWord:sub(1,1) == strippedWord:sub(1,1):upper())
                        if firstIsUpper and #gruntWord > 0 then
                            gruntWord = gruntWord:sub(1,1):upper() .. gruntWord:sub(2)
                        end
                    end
                    runelang[i] = punctuationStart .. gruntWord .. punctuationEnd
                end
                translatedText = table.concat(runelang, " ")
            elseif lang == "highglazic" then
                local glazelang = {}  
                for i, word in ipairs(words) do
                    local punctuationStart = word:match("^[%p%c]+") or ""
                    local punctuationEnd = word:match("[%p%c]+$") or ""
                    local strippedWord = word:sub(#punctuationStart + 1, #word - #punctuationEnd)
                    local lowerWord = strippedWord:lower()
                    local glazeWord = cwLanguage.glaze_dict[lowerWord]
                    if not glazeWord then
                        local syllableCount = math.random(2, 3)
                        local segments = {}
                        for j = 1, syllableCount do
                            table.insert(segments, cwLanguage.glaze_syllables[math.random(#cwLanguage.glaze_syllables)])
                        end
                        glazeWord = table.concat(segments)
                    end
                    if #strippedWord > 0 then
                        local firstIsUpper = (strippedWord:sub(1,1) == strippedWord:sub(1,1):upper())
                        if firstIsUpper and #glazeWord > 0 then
                            glazeWord = glazeWord:sub(1,1):upper() .. glazeWord:sub(2)
                        end
                    end
                    glazelang[i] = punctuationStart .. glazeWord .. punctuationEnd
                end
                translatedText = table.concat(glazelang, " ")
            elseif lang == "darkspeak" then
                local darklang = {}  -- build a new table
                for i, word in ipairs(words) do
                    local punctuationStart = word:match("^[%p%c]+") or ""
                    local punctuationEnd   = word:match("[%p%c]+$") or ""
                    local strippedWord = word:sub(#punctuationStart + 1, #word - #punctuationEnd)
                    local lowerWord    = strippedWord:lower()
                    local darkWord = cwLanguage.dark_dict[lowerWord]
                    if not darkWord then
                        local syllableCount = math.random(2, 3)
                        local segments = {}
                        for j = 1, syllableCount do
                            table.insert(segments, cwLanguage.dark_syllables[math.random(#cwLanguage.dark_syllables)])
                        end
                        darkWord = table.concat(segments)
                    end
                    if #strippedWord > 0 then
                        local firstIsUpper = (strippedWord:sub(1,1) == strippedWord:sub(1,1):upper())
                        if firstIsUpper and #darkWord > 0 then
                            darkWord = darkWord:sub(1,1):upper() .. darkWord:sub(2)
                        end
                    end
                    darklang[i] = punctuationStart .. darkWord .. punctuationEnd
                end
                translatedText = table.concat(darklang, " ")
            end

            local knowers = {}    -- those who should get the original text
            local nonknowers = {} -- those who should get the translation
            
            for _, v in ipairs(info.listeners or {}) do
                if v == speaker then
                    table.insert(knowers, v)
                else
                    if langMap[lang] == v:GetFaction() or v:HasBelief("anthropologist") or Clockwork.player:HasFlags(v, "l") or (lang == v.speakingLanguage) then
                        table.insert(knowers, v)
                    else
                        table.insert(nonknowers, v)
                    end
                end
            end
            
            -- Send the original message to those who understand.		
            info.listeners = knowers


            --print("ORIGINAL MESSAGE:", info.text)

            for _, v in ipairs(nonknowers) do
                local newInfo = table.Copy(info)  
                newInfo.text = translatedText    
            
            
                newInfo.data = newInfo.data or {}
                newInfo.data.translated = true
                --print("NEW MESSAGE:", newInfo.text)
                
                Clockwork.chatBox:Add(v, speaker, newInfo.class, newInfo.text, newInfo.data)
            end
        end
...
function Schema:PlayerCharacterLoaded(player)
	...
	if subfaction == "Clan Grock" or player:HasTrait("giant") then
	...
	player:SetSharedVar("southlander", player:GetCharacterData("southlander", false));

	if player:GetCharacterData("scrapperGang") then
		local gang = player:GetCharacterData("scrapperGang");
		player:SetSharedVar("scrapperGang", gang);
	else
...
function Schema:EntityTakeDamageNew(entity, damageInfo)
	...
	if entity:IsPlayer() and entity:HasInitialized() and entity:Alive() then
		...
		elseif damageInfo:IsDamageType(DMG_BULLET) or damageInfo:IsDamageType(DMG_BUCKSHOT) then
			...
			if cwBeliefs and entity.HasBelief and entity:HasBelief("nomadicgrock") then
				damageInfo:ScaleDamage(0.65);
			end
		
			if subfaction == "Philimaxio" or subfaction == "Knights of Sol" or subfaction == "Immortal" then
	...
	if (bIsPlayer or bIsPlayerRagdoll or entity.isTrainingDummy) and IsValid(attacker) and attacker:IsPlayer() then
		if IsValid(inflictor) then
			local coin = attacker:GetCash();
			if coin >= 5000 then
				coin = 5000;
			end

			if IsValid(inflictor) and inflictor.IsABegottenMelee and attacker:GetSubfaction() == "Yihnari" then
				damageInfo:ScaleDamage(1 + (coin / 10000));
			end
		...
		if entity.banners then
			for k, v in pairs(entity.banners) do
				if v == "glazic" then
					...
					if faction == "Gatekeeper" or faction == "Holy Hierarchy" or faction == "Hillkeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Order of Eternal Penance"  then
...
function Schema:ModifyPlayerSpeed(player, infoTable, action)
	...
	if subfaction == "Philimaxio" or subfaction == "Knights of Sol" or player:HasBelief("sows_belief3")  then
	...
	elseif subfaction == "Praeventor" or subfaction == "Outrider" or subfaction == "Nehemian"  then
	...
	elseif action == "reloading" or action == "building" or action == "skinning" or action == "mutilating" or action == "putting_on_armor" or action == "taking_off_armor" or action == "readypacking" or action == "readyball" or action == "readycan" or action == "entervehicle" then
...
function Schema:CanHearClass(listener, speaker, class)
	if table.HasValue(self.icClasses, class) then
		if listener:GetRagdollState() == RAGDOLL_KNOCKEDOUT or listener:HasTrait("deaf") then
			return false;
		end
	end
end
