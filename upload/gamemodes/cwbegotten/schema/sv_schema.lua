...
// not relevant for prelude - sprite
...
Clockwork.config:Add("discord_url", "https://discord.com/invite/wjsq4hb2CE", true);
...
function Schema:UnPermaKillPlayer(player)
	...
	if (player:GetCharacterData("permakilled")) then
		...
		Clockwork.player:SaveCharacter(player);
		hook.Run("OnPlayerUnPermaKilled", player);
	end;
end;
...
function Schema:CheapleCaughtPlayer(player)
	if IsValid(player) and player:Alive() then
		...
		if game.GetMap() == "rp_begotten3" or game.GetMap() == "rp_district21" or game.GetMap() == "bg_prelude" then
...
-- A function to collar or uncollar a player.
function Schema:CollarPlayer(player, isCollared, applier, frequency)
	if (isCollared) then
		player:SetNetVar("collared", 1);
	else
		player:SetNetVar("collared", 0);
	end;
	
	if (isCollared) then
		frequency = frequency or tostring(math.random(0,100))
		Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, player:Name().." has been collared by "..applier:Name());
		local collarEnt = ents.Create("cw_bombcollar")
		player:SetNWEntity("collarEnt", collarEnt)
		collarEnt:SetPos(player:GetPos())
		collarEnt:Spawn()
		collarEnt:AttachToPlayer(player, applier, frequency)
		collarEnt.Attached = true
		
		player:SetCharacterData("collared", true);
	end;
end;
...
function playerMeta:AddBounty(bounty, reason, poster)
	if IsValid(poster) and poster:IsPlayer() then
		...
		if (faction  == "Gatekeeper" or faction == "Hillkeeper" or faction  == "Holy Hierarchy" or faction == "Order of Eternal Penance") and !poster:IsAdmin() then
...
function playerMeta:CommitSuicideGun()

	local weapon = self:GetActiveWeapon();
	local gunshot = weapon.Primary.Sound;
	weapon = item.GetByWeapon(weapon);
	local raised = Clockwork.player:GetWeaponRaised(self);

	local gunName = weapon.name;
	...
	local possessive = "his";
	local selfless = "himself";
	local goreModel = "models/begotten/heads/male_gorecap.mdl"

	if (self:GetGender() == GENDER_FEMALE) then
		thirdPerson = "her";
		gender = "She";
		genderlc = "she";
		possessive = "her";
		selfless = "herself";
		goreModel = "models/begotten/heads/female_gorecap.mdl"
	end;

	local suicideMessages = {
		"presses the barrel of "..possessive.." "..gunName.." to "..possessive.." chin and pulls the fucking trigger!",
		"shoves the cold steel of "..possessive.." "..gunName.."'s barrel into "..possessive.." mouth before turning "..possessive.." head into a maroon water sprout!",
		"decides to paint the ceiling with "..possessive.." "..gunName.." as the brush!",
	}

	suicideMessages = table.Random(suicideMessages);

	if weapon.category == "Firearms" then
		if raised then
			local ammo = weapon:GetData("Ammo");
			if #ammo >= 1 then
				print("ammo detected 4 gun suicide")
				Clockwork.chatBox:AddInTargetRadius(self, "me", suicideMessages, self:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)

				timer.Simple(0.5, function()
					self:DeathCauseOverride("Blew "..possessive.." head off with a "..gunName..".");

					if ammo[1] ~= "Scrapshot" and ammo[1] ~= "Pop-a-Shot" and ammo[1] ~= "Volt Projectile" then
						self:SetModel(goreModel);
					end

					self:EmitSound(gunshot);
					self:Kill();
					local ragdoll = self:GetRagdollEntity();

					if ammo[1] ~= "Scrapshot" and ammo[1] ~= "Pop-a-Shot" and ammo[1] ~= "Volt Projectile" then
						if IsValid(ragdoll) then
							ragdoll:SetModel(goreModel);
						end
					end

					table.remove(ammo, 1);
					weapon:SetData("Ammo", ammo)
				end)
			else
				Schema:EasyText(self, "firebrick", "You do not have a bullet loaded to commit suicide with!")
			end
		end
	end
end;
...
function Schema:ModifyHookahTreasury(amount)
	if !self.hookahTreasury then
		self.hookahTreasury = 0;
	end
	
	self.hookahTreasury = self.hookahTreasury + amount;
end

function Schema:AddGang(name, leaderName)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end

	self.scrapperGangs[name] = {memberCount = 1, leader = leaderName, members = {}};
end;

function Schema:RemoveGang(gang)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end

	if self.scrapperGangs[gang] then
		self.scrapperGangs[gang] = nil;
	end
end;

function Schema:UpdateGangMemberCount(gang, num)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end;

	if self.scrapperGangs[gang] then
		self.scrapperGangs[gang].memberCount = self.scrapperGangs[gang].memberCount + num;
	end
end;

function Schema:UpdateMembers(gang, members)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end;

	if self.scrapperGangs[gang] then
		self.scrapperGangs[gangs].members = members;
	end
end;

function Schema:GetMemberCount(gang)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end;

	if self.scrapperGangs[gang] then
		return self.scrapperGangs[gang].memberCount;
	end
end;

function Schema:GetMembers(gang)
	if not self.scrapperGangs then
		self.scrapperGangs = {};
	end;

	if self.scrapperGangs[gang] then
		return self.scrapperGangs[gang].members;
	end
end;
...
concommand.Add("cw_CoinslotSalaryCheck", function(player, cmd, args)
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_coinslot") then
			...
			if (faction == "Gatekeeper" or faction == "Hillkeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Holy Hierarchy" or faction == "Order of Eternal Penance") then
...
concommand.Add("cw_HookahCoin", function(player, cmd, args)
	local trace = player:GetEyeTrace();
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_hookah") then
			if player:IsAdmin() then
				Schema:EasyText(player, "lightslateblue", "The hookah treasury currently sits at "..Schema.hookahTreasury.." coin.");
			end
		end
	end;
end);
...
function Schema:InSpeakerZone(entity)
	...
	elseif map == "bg_prelude" then
		return entity:GetPos():WithinAABox(Vector(3827, 10088, 4429), Vector(-1579, 18920, 646));
	elseif map == "rp_begotten_redux" or map == "rp_scraptown" then
